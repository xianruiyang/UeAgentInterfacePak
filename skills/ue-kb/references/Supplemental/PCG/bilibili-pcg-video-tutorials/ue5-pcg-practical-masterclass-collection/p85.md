...
- `PCG`、`Blueprint`、`Static Mesh`、`Mesh`、`Point Filter`、`Spline`、`Transform`、`Point`、`Attribute`、`Actor`、`Component`、`Spawn`、`Grid`、`Bounds`、`Density`、`Random`、`Seed`、`Loop`、`PCG Graph`、`PCG Component`、`Spline Sampler`、`Volume`、`Static Mesh Spawner`、`Spline Mesh`

## 分段知识

### 00:00:00-00:04:13 Spline 采样与路径生成
...
- 核对对象：`PCG Graph`、`PCG`、`Point`、`Bounds`、`Spline`、`Spline Sampler`、`Volume`、`Actor`、`Random`、`采样`。
...
### 00:04:13-00:09:11 Spline 采样与路径生成
...
- 复现要点：Spline 流程要单独核对采样间距、切线、端点、交叉点和生成网格的对齐方式。
- 核对对象：`PCG`、`Point`、`Spline`、`Actor`、`Spline Mesh`、`Create Spline`、`Filter`、`采样`、`生成`。
...
### 00:09:11-00:12:49 Spline 采样与路径生成
...
### 00:12:49-00:15:45 Spline 采样与路径生成
...
- 复现要点：Spline 流程要单独核对采样间距、切线、端点、交叉点和生成网格的对齐方式。
- 核对对象：`PCG Graph`、`PCG`、`Point`、`Attribute`、`Bounds`、`Spline`、`Static Mesh`、`Spline Mesh`、`Create Spline`、`Random`。
...
### 00:15:45-00:20:44 Geometry Script 与 Dynamic Mesh 处理
...
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 复现要点：Spline 流程要单独核对采样间距、切线、端点、交叉点和生成网格的对齐方式。
- 核对对象：`PCG`、`Point`、`Bounds`、`Spline`、`Static Mesh`、`Static Mesh Spawner`、`Transform Points`、`Filter`、`Geometry Script`、`Dynamic Mesh`。
...
### 00:20:44-00:23:46 Spline 采样与路径生成
...
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 复现要点：Spline 流程要单独核对采样间距、切线、端点、交叉点和生成网格的对齐方式。
- 核对对象：`Point`、`Attribute`、`Density`、`Bounds`、`Spline`、`Spline Sampler`、`Static Mesh`、`Static Mesh Spawner`、`Create Spline`、`采样`。
...
- 复现要点：属性命名要稳定，过滤、分区和分支节点应保留可检查的中间数据，避免后续规则难以追踪。
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 核对对象：`PCG`、`Point`、`Attribute`、`Bounds`、`Static Mesh`、`Static Mesh Spawner`、`Filter`、`Loop`、`Random`、`生成`。
...
### 00:28:37-00:33:31 Spline 采样与路径生成
...
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 复现要点：Spline 流程要单独核对采样间距、切线、端点、交叉点和生成网格的对齐方式。
- 核对对象：`PCG`、`Point`、`Attribute`、`Bounds`、`Spline`、`Spline Sampler`、`Static Mesh`、`Static Mesh Spawner`、`Filter`、`Partition`。
...
- 复现要点：属性命名要稳定，过滤、分区和分支节点应保留可检查的中间数据，避免后续规则难以追踪。
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 核对对象：`Point`、`Attribute`、`Bounds`、`Static Mesh`、`Static Mesh Spawner`、`生成`、`蓝图`。
...
### 00:37:01-00:41:55 Spline 采样与路径生成
...
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 复现要点：Spline 流程要单独核对采样间距、切线、端点、交叉点和生成网格的对齐方式。
- 核对对象：`PCG Component`、`PCG`、`Point`、`Bounds`、`Spline`、`Blueprint`、`Seed`、`Random`、`Instance`、`采样`。
...
### 00:41:55-00:42:04 Spline 采样与路径生成
