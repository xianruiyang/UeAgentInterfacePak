---
title: "场景查询系统快速入门"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/environment-query-system-quick-start-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "Gameplay系统", "人工智能", "场景查询系统", "场景查询系统快速入门"]
---

# 场景查询系统快速入门

> 路径：虚幻引擎5.7文档 / Gameplay系统 / 人工智能 / 场景查询系统 / 场景查询系统快速入门

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/environment-query-system-quick-start-in-unreal-engine

> [!WARNING]
> 在阅读本指南之前，建议先行阅读[行为树快速入门指南](../../behavior-trees/behavior-tree-in-unreal-engine---quick-start-guide/index.md)或先了解一下虚幻引擎5中的蓝图和行为树。

**场景查询系统（EQS）**可在[行为树](../../behavior-trees/index.md)中用于通过各种[测试](../environment-query-system-node-reference/eqs-node-reference-tests/index.md)轮询场景，然后根据这些测试的结果做出如何继续的决定。EQS的一些样板用例包括：让AI角色找到避开玩家视线的掩护位置，或者找到关卡中最近的体力回复剂或弹药。

场景查询实际上由多个不同部分组成。可以从行为树调用场景查询，然后实际的场景查询将使用其[生成器](../environment-query-system-node-reference/eqs-node-reference-generators/index.md)，引用其[情境](../environment-query-system-node-reference/eqs-node-reference-contexts/index.md)，并使用其测试，从而为行为树提供权重最高的结果。
...
1. 在项目中，在 **设置（Project Settings） > 插件（Plugins）** 部分，启用 **场景查询编辑器（Environment Query Editor）**。
   ...
   ![In the Project Settings,enable the Environment Query Editor option](../../../../../assets/images/ed/ed55f09805357b428aeb43334c7faa26832be46dadc45ef73c779d08476f3ab1.jpg)

   启用EQS系统后，你就可以创建并访问与EQS相关的资源。
2. 在 **Content > ThirdPerson > Blueprints** 文件夹中，将 **BP_ThirdPersonCharacter** 复制到一个名为 **AI** 的新建文件夹。

   ![Copy the BPThirdPersonCharacter over to a new folder called AI](../../../../../assets/images/c0/c07eb33a70d623254c26d75d4862a0e18bb3a079ed033ce7dd947c9782be6140.jpg)
3. 在 **AI** 文件夹中，用 **新增（Add New）> 创建高级资源（Create Advanced Asset）> 人工智能（Artificial Intelligence）** 选项创建以下三个AI资源：

   ![In the AI folder create the following three AI assets from the Add option](../../../../../assets/images/51/51e04daee3a4afa4442493b27e85f2b66133479f8f14e7fef8907a3289d4fb4d.jpg)
...
4. 创建 **AIController** 类型的新 **蓝图类**，命名为 **AIC_Enemy**。
   ...
   ![Create a new Blueprint Class of the AIController type and call it AIC_Enemy](../../../../../assets/images/f7/f75b3bebae63284085c5845583c94a83181a0364458a081c0fd73ae950439a6f.png)
5. 创建 **EnvQueryContext_BlueprintBase** 类型的新 **蓝图类**，命名为 **EQC_PlayerContext**。

   ![Create a new Blueprint Class of the EnvQueryContextBlueprintBase type and call it EQCPlayerContext](../../../../../assets/images/e5/e546fa497352d32750f29f8ea70dc8f6e5fbc1b7d7218d5fd0124a083ede6651.png)
...
1. 打开 **EQC_PlayerContext** 资源，然后在 **我的蓝图（My Blueprint）** 面板中，覆盖 **Provide Single Actor** 函数。
   ...
   ![In the My Blueprint panel override the Provide Single Actor function](../../../../../assets/images/4f/4fcd35bc35b1f85fafd02c19327314d467a53e6ddfcf5cf01a268cd3c679345a.jpg)
...
2. 在 **Provide Single Actor** 函数中，使用 **Get Player Character** 调用作为 **结果Actor（Resulting Actor）**。
   ...
   ![Use a Get Player Character call as the Resulting Actor](../../../../../assets/images/0f/0fcae7e9316fa53c962b7551dacb0d0f72a88345dd4f6662a54dfd8f9d371f99.jpg)
...
1. 在 **EQS_FindPlayer** 中，从图表中的 **Root** 节点连出引线，添加一个 **Points:Grid** 节点。
   ...
   ![Drag off the Root node in the graph and add a Points: Grid node](../../../../../assets/images/ee/eed1463ed9738e2b95cca54ab45c887f979433eed5c3fbc5f12a53ad7bcaf4da.png)
...
2. 在 **细节（Details）** 面板中，将 **网格半大小（GridHalfSize）** 改为 **800**，**间隔（Space Between）** 改为 **150.0**。
   ...
   > 图片已省略：Change the GridHalfSize to 800 and the Space Between to 150.0 in the details panel
...
3. 右键单击 **SimpleGrid** 节点，在 **添加测试（Add Test）** 下选择 **追踪（Trace）**。
   ...
   > 图片已省略：Right-click on the SimpleGrid node and under Add Test select Trace
...
4. 右键单击并再添加一个 **距离（Distance）** 类型的测试**。**
   ...
   > 图片已省略：Right-click and add another Test of the Distance type

   在 **追踪** 测试返回能够看到玩家角色的点之后，**距离** 测试将用于根据这些点与玩家角色的接近程度来对其计分。
5. 选择 **追踪** 测试，然后在 **细节（Details）** 面板中设置下列选项：

   > 图片已省略：Select the Trace Test and in the Details panel set the following options
...
6. 选择 **距离** 测试，然后在 **细节（Details）** 面板中，将 **测试目的（Test Purpose）** 改为 **仅计分（Score Only）**，**计分因子（Scoring Factor）** 改为 **-1.0**。
   ...
   > 图片已省略：Change the Test Purpose to Score Only and the Scoring Factor to -1.0 in the Details panel
...
1. 在 **BB_Enemy** 黑板资源中，创建以下三个键：
   ...
   > 图片已省略：Create the following three Keys in the BB_Enemy Blackboard asset
...
1. 从 **Patrolling** 序列节点连出引线，添加一个 **移动到（Move To）** 任务（设为 **MoveToLocation**）和一个 **等待（Wait）** 任务（设为 **5** +/- **1** 秒）。
   ...
   > 图片已省略：Add a Move To Task and a Wait Task oiff the Patrolling Sequence node

   这将指示AI移动到黑板键 **MoveToLocation** 然后等待指定的时间，但我们还需要为 **MoveToLocation** 定义矢量值。
2. 从工具栏创建 **新任务**，然后在 **内容浏览器** 中将它命名为 **BTT_RandomLocation**。

   > 图片已省略：From the Toolbar create a New Task then in the Content Drawer call it BTT_RandomLocation
3. 在 **BTT_RandomLocation** 中，重建如下所示的蓝图图表。
   ...
   >
   > > 图片已省略：Click the eye icon in the My Blueprint panel
4. 在行为树中，添加 **BTT_RandomLocation** 任务（在 **细节（Details）** 面板中设置 **MoveToLocation**）作为 **Patrolling** 下面的第一个节点。

   > 图片已省略：Add the BTT_RandomLocation Task in the Behavior Tree
...
1. 右键单击 **In Combat** 选择器并添加 **黑板** 类型的 **装饰器**，并使用下列设置：
   ...
   > 图片已省略：Right-click on the In Combat Selector and add a Decorator of the Blackboard type
...
2. 右键单击 **Attack** 序列并添加 **黑板** 类型的 **装饰器**，并使用下列设置：
   ...
   > 图片已省略：Right-click on the Attack Sequence and add a Decorator of the Blackboard type

   - 通知观察者（Notify Observer）

     设为

     数值改变时（On Value Change）
   - 观察者中止（Observer aborts）
...
3. 从 **Attack** 节点连出引线，添加一个 **旋转以面向BB（Rotate to face BB）** 条目（设为 **TargetActor**）。
   ...
   > 图片已省略：Off the Attack node, add a Rotate to face BB entry

   这将使AI在处于"attack"分支时转向TargetActor。在这个示例中，我们没有给AI提供攻击，如果需要，可以稍后添加。
4. 从 **Move Into Position** 节点连出引线，使用 **Run EQSQuery** 节点。

   > 图片已省略：Off the Move Into Position node use the Run EQSQuery node
...
5. 在 **Run EQSQuery** 节点的 **细节（Details）** 面板中，将 **黑板键（Blackboard Key** 设为 **MoveToLocation**，**查询模板（Query Template）** 设为 **EQS_FindPlayer**。
   ...
   > 图片已省略：Set the Blackboard Key to MoveToLocation and the Query Template to EQSFindPlayer
6. 添加一个 **MoveTo**（设为 **MoveToLocation）**，并添加 **旋转以面向BB（Rotate to face BB）** 条目（设为 **TargetActor**）跟随 **Run EQS Query。**

   > 图片已省略：Add a MoveTo and Rotate to face BB entry following the Run EQS Query
...
1. 在
   ...
   占据时事件（Event On Possess）

   并连接到

   运行行为树（Run Behavior Tree）
...
1. 添加 **AI感知（AIPerception）** 组件，并使用下列 **AI视觉配置（AI Sight config）** 设置：
   ...
   > 图片已省略：Add an AIPerception component with the following AI Sight config settings

   - 感官配置（Senses Config）

     ，添加

     AI视觉配置（AI Sight config）
   - 探测中立方（Detect Neutrals）
...
4. 从第2个分支的 **False** 引脚连出引线，添加如下所示的三个节点：
   ...
   > 图片已省略：Off the False pin of the 2nd Branch, add the 3 nodes
...
5. 在 **我的蓝图（My Blueprint）** 面板中创建两个函数，名为：**UpdateSightKey** 和 **UpdateTargetKey**。
   ...
   > 图片已省略：Create UpdateSightKey and UpdateTargetKey Functions in the My Blueprint panel

   我们将使用这两个函数更新我们在行为树中用于作出决定的黑板键。
6. 针对 **UpdateSightKey** 添加一个 **布尔** 输入，名为 **HasLineOfSight**。

   > 图片已省略：Add a Bool input called HasLineOfSight
7. 在 **UpdateSightKey** 中右键单击并获取 **黑板（Blackboard）** 变量，然后 **将值设为布尔（Set Value as Bool）**，**键名（Key Name）** 使用 **HasLineOfSight**。

   > 图片已省略：In UpdateSightKey right-click and get the Blackboard variable and Set Value as Bool with Key Name using HasLineOfSight

   这样就可以使用这个函数将布尔值传递到我们的黑板键，更新 **HasLineOfSIght** 键。
8. 针对 **UpdateTargetKey**，添加一个 **Actor** 输入，名为 **TargetActor**。

   > 图片已省略：Add an Actor input called TargetActor for UpdateTargetKey
9. 在 **UpdateTargetKey** 中右键单击并获取 **黑板（Blackboard）** 变量，然后 **将值设为对象（Set Value as Object）**，**键名（Key Name）** 使用 **TargetActor**。

   > 图片已省略：In UpdateTargetKey right-click and get the Blackboard variable and Set Value as Object with Key Name using TargetActor

   此函数与UpdateSightKey函数类似，用于使用我们传递的任何Actor来更新黑板键 **TargetActor**。
10. 将 **UpdateSightKey** 和 **UpdateTargetActor**函数添加到 **False** 条件，如下所示：

    > 图片已省略：Add UpdateSightKey and UpdateTargetActor functions to the False condition
...
11. 从第2个分支的 **True** 引脚连出引线，使用 **LostSIghtTimer** 和 **Clear and Invalidate Timer by Handle**。
    ...
    > 图片已省略：Off the True pin of the 2nd Branch use LostSIghtTimer and Clear and Invalidate Timer by Handle

    这将会停止和重置失去对玩家的视线时使用的定时器。
12. 添加 **UpdateSightKey**（设为 **启用（enabled）**）和 **UpdateTargetKey**（设为 **感知的Actor（Perceived Actor）**。

    > 图片已省略：Add the UpdateSightKey and UpdateTargetKey
...
2. 在 **模式（Modes）** 面板中，向关卡添加 **寻路网格体包围体（Nav Mesh Bounds Volume）**，并调整其大小，使其包住整个关卡。
   ...
   > 图片已省略：Add a Nav Mesh Bounds Volume to the Level and scale it so it encapsulates the Level from the Modes panel
...
3. 右键单击关卡中的 **BP_ThirdPersonCharacter**，然后选择 **编辑第三人称角色（Edit BP_hirdPersonCharacter）**。
   ...
   > 图片已省略：Right-click on the BPThirdPersonCharacter in the Level, then select Edit BPThirdPersonCharacter
4. 在 **细节（Details）** 面板中搜索 **标记（Tag）**，然后添加一个名为 **Player** 的 **标记**。

   > 图片已省略：Search for Tag, then add a Tag called Player in the Details panel

   在我们的 **AIC_Enemy** 蓝图中，当AI感知系统感知到Actor时，因为这个Actor有Player标记，所以我们的分支求值结果将是True。
5. 在关卡中，放大并添加多个版本的 **立方体网格体**，提供更多会遮挡视线的位置。

   > 图片已省略：Scale up and add multiple versions of the Cube Mesh to provide additional cover points to break line of sight
...
可以使用[AI调试工具](../../ai-debugging/index.md)查看任何活动的EQS查询（除了行为树或感知信息之外）。要在运行时激活AI调试，请按 **'**（撇号）键，然后选择1（进行一般AI调试）、2（用于行为树）、3（用于EQS）或4（用于AI感知）。下面我们激活AI调试并调出EQS调试工具。
...
> 图片已省略：You can create EQS Testing Pawn by creating a new Blueprint Class of the EQS Testing Pawn Class

我们的当前设置使用玩家角色作为情境，用于在我们的EQS测试中求值。要在游戏未运行时测试，我们需要对 **EQS_PlayerContext** 蓝图稍加修改，覆盖 **Provide Actors Set** 函数。

> 图片已省略：We need to make a slight modification to the EQS_PlayerContext Blueprint and override the Provide Actors Set function

我们可以使用 **获取所有（Get All Actors of Class）**，将其设为 **ThirdPersonCharacter**，这会提供 **结果Actor集（Resulting Actors Set）**：

> 图片已省略：We can use Get All Actors of Class set to BP_ThirdPersonCharacter which provides the Resulting Actors Set

将EQS测试Pawn添加到关卡时，在 **细节（Details）** 面板中可以指定 **查询模板（Query Template）**（我们已将它设为我们的 **EQS_FindPlayer** 查询）。

> 图片已省略：You can assign the Query Template in the Details panel,
