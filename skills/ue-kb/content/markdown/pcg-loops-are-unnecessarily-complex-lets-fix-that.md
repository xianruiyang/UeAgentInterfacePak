...
- 围绕“PCG Loops Are Unnecessarily Complex. Lets Fix That!”整理 PCG 视频中的输入数据、图表规则、关键节点、参数风险和最终生成结果。
...
- `PCG`、`Blueprint`、`Static Mesh`、`Mesh`、`Spline`、`Transform`、`Point`、`Attribute`、`Spawn`、`Bounds`、`Random`、`Seed`、`Loop`、`Graph`、`Material`、`Landscape`、`points`、`PCG Graph`、`Point Data`、`Spline Sampler`、`Static Mesh Spawner`、`Transform Points`、`Filter`、`Partition`
...
- 核对对象：`PCG Graph`、`PCG`、`Point`、`Attribute`、`Blueprint`、`Static Mesh`、`Static Mesh Spawner`、`Transform Points`、`Loop`、`Random`。
...
### 00:04:35-00:09:24 Spline 采样与路径生成
...
- 核对对象：`PCG`、`Point Data`、`Point`、`Attribute`、`Spline`、`Spline Sampler`、`Static Mesh`、`Static Mesh Spawner`、`Partition`、`Loop`。
...
### 00:09:24-00:14:22 Geometry Script 与 Dynamic Mesh 处理
...
- 核对对象：`Point`、`Attribute`、`Static Mesh`、`Transform Points`、`Filter`、`Partition`、`Loop`、`Geometry Script`、`Dynamic Mesh`、`Boolean`。
...
- 复现要点：先用 Debug/Inspect 核对点数量、Bounds、Density 和关键属性，再判断最终生成结果。
- 复现要点：属性命名要稳定，过滤、分区和分支节点应保留可检查的中间数据，避免后续规则难以追踪。
- 核对对象：`PCG`、`Point`、`Attribute`、`Loop`、`属性`、`过滤`。
