---
title: "虚幻编辑器中的版本控制"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/using-source-control-in-the-unreal-editor"
breadcrumbs: ["虚幻引擎5.7文档", "建立你的开发流程", "虚幻引擎中的协作和版本控制", "虚幻编辑器中的版本控制"]
---

# 虚幻编辑器中的版本控制

> 路径：虚幻引擎5.7文档 / 建立你的开发流程 / 虚幻引擎中的协作和版本控制 / 虚幻编辑器中的版本控制

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/using-source-control-in-the-unreal-editor
...
协作的主要方式是通过 [内容浏览器](../../../understanding-the-basics/content-browser/index.md)。所有的版本控制的操作都在文件或者目录的菜单选项中。比如，要 **Check Out** 一个文件（为了把它锁定并编辑）， 在 **内容浏览器** 中对该资源 **点击右键** 并选择 **Check Out** 项目。
...
> [!NOTE]
> 当一个文件是 **Out of Sync** 时（比如版本管理软件服务器上有一个比你本地文件更新的文件版本），在 **内容浏览器** 中这个按钮，， 将会显示在该资源上。
...
> [!NOTE]
> 在 **内容浏览器** 中，新增文件会有一个 标记并能够被签入，和文件被 Check Out 的行为类似。
...
一旦选定了如何处理删除的动作，该文件通常会被一个隐藏的 **资源** 替换，该资源叫做 [重定向链接](../../asset-redirectors/index.md)。如果想要清理这些由于删除资源而至的 [重定向链接](../../asset-redirectors/index.md)。如果你想在删除资产时清理重定向器，你可以在 **内容浏览器** 内的某个 **文件夹目录** 上 **点击右键** 并选择 **修复目录的重定向链接 Fix Up Redirectors in Folders**。这将会修复项目中对重定向有引用关系的资源素材，并删除那些重定向链接，并在版本管理软件中标记这些重定向链接文件为删除状态。要提交这个改变的话，仍然需要借助外部工具，比如 P4V 或者 TortoiseSVN 来完成提交。
