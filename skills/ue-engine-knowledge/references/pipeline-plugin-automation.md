# Pipeline / Plugin / Editor Automation 知识卡

## 适用范围

插件、Editor automation、Commandlet、资产批处理、CI 构建、打包和脚本化验证。
...
- 自动化写入资产后必须保存、编译或读回验证。
- 大批量修改前先做小样本、日志、回滚和 dry-run。
- 插件代码变更需要处理 UE 占用 DLL 的情况。
- 脚本不要依赖当前窗口焦点或全屏运行。
