# SaveGame / Asset Manager 知识卡

## 适用范围

SaveGame、持久化、Primary Asset、Asset Manager、异步加载、软引用和资产注册。
...
- SaveGame 存稳定 ID 和状态，不存临时对象指针。
- 大资源用 Soft Reference，通过 Asset Manager 异步加载。
- Primary Asset 类型、ID、目录规则要稳定。
- 读档流程要处理版本升级、缺失资产和加载失败。
