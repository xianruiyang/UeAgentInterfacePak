---
title: "Actor Sequence组件"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/sequencer-blueprint-component-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "为角色和对象制作动画", "过场动画和Sequencer", "Sequencer概述", "Actor Sequence组件"]
---

# Actor Sequence组件

> 路径：虚幻引擎5.7文档 / 为角色和对象制作动画 / 过场动画和Sequencer / Sequencer概述 / Actor Sequence组件

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/sequencer-blueprint-component-in-unreal-engine

> [!WARNING]
> 这是尚处于开发阶段的实验性功能。一些方面可能无法正常使用，或会在未来版本中被修改。

使用并创建 [序列](../index.md) 时，有时可能需要在其他地方或实例中重复使用序列的功能。 借助 **Actor 序列插件** 和 **Actor 序列组件** 便可将序列直接嵌入 Actor [蓝图](../../../../blueprints-visual-scripting/index.md)。 将动画绑定到蓝图实例并自动触发或通过蓝图的 **事件图表** 触发，即可重复使用序列。 此外还可为世界场景中的任意 Actor 添加 Actor 序列组件，将动画添加到 Actor 的单一实例。
...
1. 项目打开后，从 **Edit** 菜单中选择 **插件**。
2. 从 **插件** 菜单的 **Built-in** 下启用 **Actor Sequence Editor** 选项，并在弹出提示后重启编辑器。

   > [!NOTE]
   > 启用 Actor 序列编辑器时可能出现 **Experimental** 确认对话窗口，点击 **Yes** 继续。
3. 创建一个 **Actor** 类的新 **蓝图**，将其命名为 **Light_BP**。
4. 在 **Light_BP** 中点击 **Add Component** 并添加一个 **Spot Light**。
5. 在 **Spot Light** 的 **Details** 面板中将 **Y** 轴的 **Rotation** 值改为 **-60**。

   这将使光线在视口中旋转，角度稍微朝下。
6. 添加另一个 **Static Mesh** 类型的 **组件**，然后在 **Details** 面板中将网格体设为 **Floor_400x400**，**Location** 设为 **-180 (X), -180 (Y), -100 (Z)**。
...
23. 在 **Content Browser** 中，在关卡中拖动并放置 **Light_BP** 实例（如有需要可以按下 **E** 并进行旋转）。
