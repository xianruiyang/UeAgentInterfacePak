---
title: "在线子系统成就接口"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/online-subsystem-achievements-interface-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "Gameplay系统", "在线子系统和服务", "在线子系统", "在线子系统成就接口"]
---

# 在线子系统成就接口

> 路径：虚幻引擎5.7文档 / Gameplay系统 / 在线子系统和服务 / 在线子系统 / 在线子系统成就接口

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/online-subsystem-achievements-interface-in-unreal-engine
...
数据被缓存后，即可通过几个函数调用对其进行检查。 `GetCachedAchievements` 是最佳的上手方法，因为其将返回一个包含所有已知成就ID的数组。 之后这些ID可用于调用 `GetCachedAchievement`（带特定玩家的 `FUniqueNetId`），以便检查此玩家在该成就上的完成百分比；或者作为一个参数来 `GetCachedAchievementDescription`，其将返回完整的成就描述（类型为[`FOnlineAchievementDesc`](https://dev.epicgames.com/documentation/404)）。
