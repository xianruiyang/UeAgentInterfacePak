---
title: "编辑枢轴点"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/edit-pivot-tool-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "管理内容", "建模和几何体脚本编写", "建模工具", "编辑枢轴点"]
---

# 编辑枢轴点

> 路径：虚幻引擎5.7文档 / 管理内容 / 建模和几何体脚本编写 / 建模工具 / 编辑枢轴点

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/edit-pivot-tool-in-unreal-engine
...
你可以从 **建模模式（Modeling Mode）** 中的 **Xform** （变换）类别访问编辑枢轴点工具。如需详细了解建模模式以及访问方法，请参阅[建模模式概述](../../getting-started-with-modeling-mode/modeling-mode/index.md)。
...
当你接受新位置时，该位置将被烘焙到网格体中，这意味着该位置将应用于到静态网格体资产和场景中的实例。为避免更改静态网格体实例的资产和位置，你可以复制网格体，使用[复制](../index.md#xform)工具创建新的Actor。
...
| **旋转模式（Rotation Mode）** | 确定在对齐拖动过程中如何旋转枢轴点。你可以从以下选项中选择： **忽略（Ignore）** ：仅平移枢轴点。 **对齐（Align）** ：将枢轴点的Z轴和网格体的法线对齐，以指向同一方向。 **对齐翻转（Align Flipped）** ：将枢轴点的Z轴与网格体的相反法线对齐。 |

要以数字方式改变枢轴点的位置和旋转，请使用 **变换面板（Transform Panel）** 。如需详细了解建模小工具以及如何访问变换面板，请参阅概述文档的[小工具](../../getting-started-with-modeling-mode/modeling-mode/index.md#%E5%B0%8F%E5%B7%A5%E5%85%B7)小节。

你可以使用 **盒体位置（Box Positions）** 分段中的预设，自动将枢轴点对齐到某个位置。这些位置基于你在 **使用世界盒体（Use World Box）** 属性中选择的边界盒体的轴。

> [!TIP]
> 为了帮助确定枢轴点的方向，将小工具的颜色与[关卡编辑器](../../../../building-virtual-worlds/level-editor/index.md)的轴控件进行比较。
