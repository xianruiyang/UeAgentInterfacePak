# Data Assets Are Great in PCG, But Data Tables Offer a Unique Benefit!

# Data Assets Are Great in PCG, But Data Tables Offer a Unique Benefit!

## 知识目标

- 围绕“Data Assets Are Great in PCG, But Data Tables Offer a Unique Benefit!”整理 PCG 视频中的输入数据、图表规则、关键节点、参数风险和最终生成结果。
...
- `PCG`、`Blueprint`、`Static Mesh`、`Mesh`、`Transform`、`Point`、`Attribute`、`Component`、`Spawn`、`Grid`、`Bounds`、`Density`、`Random`、`Loop`、`Graph`、`data`、`table`、`PCG Graph`、`Volume`、`Static Mesh Spawner`、`Transform Points`、`Filter`、`Partition`、`Boolean`

## 分段知识

### 00:00:00-00:04:16 Geometry Script 与 Dynamic Mesh 处理
...
- 核对对象：`PCG Graph`、`PCG`、`Blueprint`、`Static Mesh`、`Random`、`Geometry Script`、`Dynamic Mesh`、`Boolean`、`Data Asset`、`Data Table`。
...
- 核对对象：`PCG Graph`、`PCG`、`Point`、`Attribute`、`Density`、`Bounds`、`Volume`、`Static Mesh`、`Static Mesh Spawner`、`Transform Points`。
...
- 复现要点：属性命名要稳定，过滤、分区和分支节点应保留可检查的中间数据，避免后续规则难以追踪。
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 核对对象：`PCG Graph`、`PCG`、`Point`、`Attribute`、`Bounds`、`Static Mesh`、`Static Mesh Spawner`、`Loop`、`Data Asset`、`Data Table`。
...
- 复现要点：先用 Debug/Inspect 核对点数量、Bounds、Density 和关键属性，再判断最终生成结果。
- 复现要点：属性命名要稳定，过滤、分区和分支节点应保留可检查的中间数据，避免后续规则难以追踪。
- 核对对象：`PCG`、`Point`、`Attribute`、`Data Asset`、`Data Table`、`属性`、`过滤`。
