---
title: "空间化概述"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/spatialization-overview-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "处理音频", "空间化和声音衰减", "空间化概述"]
---

# 空间化概述

> 路径：虚幻引擎5.7文档 / 处理音频 / 空间化和声音衰减 / 空间化概述

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/spatialization-overview-in-unreal-engine
...
| **衰减** | 听者与声源之间的距离。 |
| **传播** | 声音从声源到听者的路径。 |
| **阻塞和遮挡** | 声音在向听者传播时可能会遇到其他物体。这些物体就是声音的障碍物。 |
| **混响** | 听者可能听到从不同路径传来的声音，从而感知到回声效果。 |
...
![undefined](../../../../assets/images/88/88365ff7b3ff19e850e6249aa4a56f92ac6f53a723546656f1e92c6ad73f55f8.png)

然而，对于环绕声扬声器，布置更为复杂，取决于扬声器的数量及其放置标准。下面是5.1环绕声设置中扬声器的典型布置（未显示低音炮）：

![undefined](../../../../assets/images/30/3010cf4605bdaefaa1d53b0ff8212c7d5c87efdfff37f1bb66447133d4e0c437.png)
...
![undefined](../../../../assets/images/97/9788e86618b5a6bc68db9fb14beb77009a2129863f91bc1c92292283917f9465.png)
...
![undefined](../../../../assets/images/3f/3f0ca1dfbc5be57d9f7bc9fca08e29d508eec27fd14f3ce4844723f02c0b1265.png)

对于高于立体声的声道源，通常是偶数数量（4、6或8），可以采用类似的算法。不像扩散参数是定义为与左右声道的距离（或半距离），它们定义为以源位置为中心的圆的半径。在这种情况下，源声道虚拟点通常是圆边缘上的点，均匀分布。

![undefined](../../../../assets/images/21/210df68b3a237eb1f24ff85c44b5500d30b7ab02d03d69eb008b14efae760798.png)
...
GainLeft + GainRight = 1 GainLeft = X GainRight = 1 - X

可视化的线性平移如下所示：

![undefined](../../../../assets/images/c4/c42ecf88364eda19cee1623eab13394b0a317686b74f1d1359b01a2b41f8aa4b.png)

线性平移很容易计算，但它有一个根本的缺点：响度（音量）是用来描述声音幅度的感知体验的术语，它不是由音频信号的实际幅度决定的，而是由音频信号的功率决定的。功率等于信号幅度的平方。如果声音的幅度或增益为X，则平移计算的功率由以下公式计算：

PowerLeft = GainLeft2 = X2 PowerRight = GainRight2 = (1 - X)2

总功率等于计算出的左右声道的功率之和：

PowerTotal = GainLeft2 + GainRight2 PowerTotal = X2 + (1 - X)2

在此公式中使用增益值表明功率不是恒定的。当声音在中间（X = 0.5）相对于侧面（X = 0.0或X = 1.0）进行平移时，功率和响度将下降。

下面是左右声道之间的平移中心的功率或感知响度：

PowerTotal = (0.5)2 + (1 - 0.5)2 PowerTotal = 0.25 + 0.25 PowerTotal = 0.5

下面是声音在右侧或左侧时：

PowerTotal = (1)2 + (1 - 1)2 PowerTotal = 1 + 0 PowerTotal = 1
...
GainLeft = √X GainRight = √(1 - X)

可以看出总功率始终保持不变：

PowerTotal = (√X)2 + (√(1-X))2 PowerTotal = X + (1-X) PowerTotal = 1

对于余弦平移定律，左右声道增益计算如下：

GainLeft = Sin2 (X) GainRight = Cos2 (X)
...
![undefined](../../../../assets/images/4f/4fd546bf6fe9aa6d576b9ec637d3663e0234d3f9fe33bfd36bb18629590610ff.png)
...
![undefined](../../../../assets/images/4b/4b256ba5082cdd53c859830962e733fdfaa40c6f3157fa013e252195dfec5cee.png)
