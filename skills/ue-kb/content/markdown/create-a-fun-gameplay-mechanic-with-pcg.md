# Create a FUN Gameplay Mechanic With PCG

# Create a FUN Gameplay Mechanic With PCG

## 知识目标

- 围绕“Create a FUN Gameplay Mechanic With PCG”整理 PCG 视频中的输入数据、图表规则、关键节点、参数风险和最终生成结果。
...
- `PCG`、`Blueprint`、`Static Mesh`、`Mesh`、`Spline`、`Transform`、`Point`、`Attribute`、`Actor`、`Component`、`Spawn`、`Bounds`、`Random`、`Loop`、`Graph`、`Material`、`Instance`、`collection`、`PCG Graph`、`PCG Component`、`Spline Sampler`、`Volume`、`Static Mesh Spawner`、`Spawn Actor`
...
- 核对对象：`PCG Graph`、`PCG Component`、`PCG`、`Point`、`Spline`、`Spline Sampler`、`Actor`、`Blueprint`、`Create Spline`、`Loop`。
...
- 复现要点：先用 Debug/Inspect 核对点数量、Bounds、Density 和关键属性，再判断最终生成结果。
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 核对对象：`Point`、`Actor`、`Blueprint`、`Static Mesh`、`Static Mesh Spawner`、`Transform Points`、`Random`、`Material`、`Instance`、`生成`。
...
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 核对对象：`PCG`、`Attribute`、`Actor`、`Static Mesh`、`Static Mesh Spawner`、`Spawn Actor`、`Instance`、`生成`、`蓝图`。
...
### 00:12:06-00:16:54 Geometry Script 与 Dynamic Mesh 处理
...
- 核对对象：`Attribute`、`Volume`、`Actor`、`Blueprint`、`Static Mesh`、`Static Mesh Spawner`、`Spawn Actor`、`Branch`、`Loop`、`Geometry Script`。
...
- 复现要点：先用 Debug/Inspect 核对点数量、Bounds、Density 和关键属性，再判断最终生成结果。
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 核对对象：`PCG Graph`、`PCG`、`Point`、`Actor`、`Static Mesh`、`Spawn Actor`、`Loop`、`Instance`、`生成`、`蓝图`。
...
- 知识点：Static Mesh Spawner 将点数据实例化为静态网格，复现时要检查 Mesh、Transform、Density 和材质覆盖。
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 核对对象：`Actor`、`Blueprint`、`Spawn Actor`、`Loop`、`Random`、`Material`、`Instance`、`生成`、`蓝图`。
...
- 复现要点：先用 Debug/Inspect 核对点数量、Bounds、Density 和关键属性，再判断最终生成结果。
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 核对对象：`PCG`、`Bounds`、`Actor`、`Blueprint`、`Static Mesh`、`Loop`、`Random`、`生成`、`蓝图`。
