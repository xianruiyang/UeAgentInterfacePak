---
title: "创建和使用材质实例"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/creating-and-using-material-instances-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "设计视觉、渲染和图形效果", "材质", "材质实例", "创建和使用材质实例"]
---

# 创建和使用材质实例

> 路径：虚幻引擎5.7文档 / 设计视觉、渲染和图形效果 / 材质 / 材质实例 / 创建和使用材质实例

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/creating-and-using-material-instances-in-unreal-engine
...
为了实现这种灵活性，[材质实例化](../index.md)使用了一种称为 **继承（inheritance）** 的概念：父材质的属性会传递给其子项。 父材质中指定为 **参数（parameters）** 的属性在 **材质实例编辑器（Material Instance Editor）** 中公开给美术师。

![材质实例编辑器](../../../../../assets/images/96/96c7b18df70a6d269132611a6e65685124343606e1d370fc7f2db2bb07e19df9.jpg)

使用材质实例，你可以从单个基础材质创建许多变体。

![三个材质实例](../../../../../assets/images/11/1155311997a5232a4acbb51ea4290b4bc7bd5f90ca7e5e0d04509b225718df4b.jpg)
...
2. 在内容浏览器中创建
   ...
   材质实例常量（Material Instance Constant）

   。 然后你可以在材质实例编辑器中自定义该材质。

如需进一步了解关于材质实例化的背景信息，请[阅读此处的概述](../index.md)。

## 创建参数化材质

材质实例继承其父材质的所有属性，但默认情况下并非所有这些属性都可以在材质实例编辑器中自定义。

要使材质属性在材质实例中可编辑，你必须使用参数节点而不是常规[材质表达式](../../unreal-engine-material-expressions-reference/index.md)来将特定属性指定为 **参数（parameters）** 。
...
![材质表达式控制板和上下文菜单](../../../../../assets/images/ec/ec9c77f726e3057674b305e298aced259ee5c81e89b2bb92e978be75f37b404a.png)
...
1. 标量参数（Scalar Parameter）
   ...
   以放置标量参数。 标量参数是常量的参数化版本，并且包含单个数字值。
2. 矢量参数（Vector Parameter）
   ...
   以放置矢量参数。矢量参数是Constant 4 Vector的参数化版本，并且包含四个浮点值。

使用快捷键或控制板将标量参数放置在材质图表中。

![粗糙度标量参数](../../../../../assets/images/b0/b08168832fd81bf657a54f1e1365e8e0852c92d1319690e5c08433a68a1152b6.jpg)

将该参数命名为 **粗糙度（Roughness）** 并将其连接到对应的[材质输入](../../material-inputs/index.md)。

#### 默认值和值范围

如果你想设置参数的默认值或限制值范围，可以在细节（Details）面板中这样做。

![默认值粗糙度](../../../../../assets/images/5e/5e9242946591fa815461781b0d17ed2c1d101733be6e21da20ec4760a129a83f.png)
...
![金属感和粗糙度参数](../../../../../assets/images/e9/e9d9dcc0ebb3405e3f536e4ef488c734d0d328e024b214b6eeb79becc4f87903.jpg)
...
![插入Constant3 Vector](../../../../../assets/images/e6/e61fa302921ee8cfe382736c425d3b055897d8828c4e82a189e2472244db95b2.jpg)

要将此节点转换为参数，请 **右键点击** 该节点，然后从上下文菜单中选择 **转换为参数（Convert to Parameter）**。

![转换为参数](../../../../../assets/images/f8/f88371b473efdff467c4546bba87621bdfebd403202b9a5f0d5727050e1d651e.jpg)
...
点击[材质编辑器](../../unreal-engine-material-editor-user-guide/unreal-engine-material-editor-ui/index.md)左上角的 **应用（Apply）** 以编译材质，然后点击 **保存（Save）** 。

> 图片已省略：编译材质

## 创建材质实例

现在你有了简单的参数化材质，可以创建材质实例并在[材质实例编辑器](../unreal-engine-material-instance-editor-ui/index.md)中进行自定义。
...
2. 在细节（Details）面板中，展开

   组排序（Group Sorting）

   ，然后展开

   参数组数据（Parameter Group Data）
