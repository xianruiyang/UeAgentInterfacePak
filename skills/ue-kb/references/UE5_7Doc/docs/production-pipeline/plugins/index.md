---
title: "插件"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/plugins-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "建立你的开发流程", "插件"]
---

# 插件

> 路径：虚幻引擎5.7文档 / 建立你的开发流程 / 插件

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/plugins-in-unreal-engine
...
现在如要创建插件，请参见[新建插件](#%E6%96%B0%E5%BB%BA%E6%8F%92%E4%BB%B6)章节。
...
含有代码的插件包含Source文件夹。此文件夹将包含一个或多个目录，其中包含插件的模块源代码。注意：插件常包含代码，但并非必要条件。参见[插件中的代码](#%E6%8F%92%E4%BB%B6%E4%B8%AD%E7%9A%84%E4%BB%A3%E7%A0%81)章节，了解更多信息。

含代码的插件拥有包含该插件编译后代码的 `Binaries` 文件夹，而临时版本产品文件将另存在插件目录中的 `Intermediate` 文件夹中。

插件可拥有自己的Content文件夹，其中包含特定于该插件的资源文件。参见[插件中的内容](#%E6%8F%92%E4%BB%B6%E4%B8%AD%E7%9A%84%E5%86%85%E5%AE%B9)章节，了解更多信息。插件配置文件应使用和其他配置文件相同的规则放置：
...
| 插件类型 | 搜索路径 |
| --- | --- |
| 引擎 | `[UE根目录]/Engine/Plugins/[插件命名]` |
| 游戏 | `[项目根目录]/Engine/Plugins/[插件命名]/` |

也可将插件整理到基础Plugins文件夹下的子目录中。引擎将扫描基础Plugins文件夹下的所有子文件夹，查找要加载的插件，但不会扫描已找到插件下的子目录。

虚幻引擎通过在磁盘上搜索 `.uplugin` 文件来查找插件。将此类文件称为 **插件描述文件** 。其是提供插件相关基本信息的文本文件。运行此类程勋时，引擎、编辑器和 **UnrealBuildTool** （UBT）将自动发现并自动加载插件描述文件。参见[插件描述文件](#%E6%8F%92%E4%BB%B6%E6%8F%8F%E8%BF%B0%E6%96%87%E4%BB%B6)章节，了解创建和自定义此类文件的方法。
...
插件位于项目目录的 `插件` 子文件夹下，将在引擎或编辑器启动时被探测和加载。
...
如果你要预编译UE 5.2的插件，请使用Visual Studio 2019编译你的插件，这是UE 5.2支持的最低Visual Studio版本。使用Visual Studio 2019编译可确保生成的库针对所有用户兼容UE 5.2。如需详细了解Visual Studio和虚幻引擎版本兼容性，请参阅[设置Visual Studio](https://dev.epicgames.com/documentation/404)，获取更多信息。
...
各条目须具有"Name"和"Type"字段。"Name"是插件模块的唯一命名，将随插件而加载。在运行时，插件的"Binaries"文件夹中需存在正确的插件二进制文件，并带有指定模块命名。对于具有Source目录的模块，模块的子文件夹树中须存在匹配的".Build.cs"文件。"Type"设置模块的类型。有效的选项为： **运行时** 、 **RuntimeNoCommandlet** 、 **开发者** 、 **编辑器** 、 **EditorNoCommandlet** 和 **Program** 。此类型决定能加载模块的应用程序类型。例如部分插件可能包含仅在编辑器运行时加载的模块。将在所有情况下加载运行时模块，即使在已发行的游戏中，也需加载。仅在开发运行时或编辑器版本中加载开发者模块，不会发行版本中加载。仅在编辑器启动时加载编辑器模块。插件可使用不同类型模块的组合。
