---
title: "蓝图通信用法"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/blueprint-communication-usage-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "蓝图可视化脚本", "专用蓝图节点组", "蓝图通信用法"]
---

# 蓝图通信用法

> 路径：虚幻引擎5.7文档 / 蓝图可视化脚本 / 专用蓝图节点组 / 蓝图通信用法

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/blueprint-communication-usage-in-unreal-engine
...
> [!NOTE]
> 在 [设置直接蓝图交流](../../../gameplay-systems/gameplay-framework/actors/actor-communication/index.md)中可查阅设置流程。

## 事件分配器

**事件分配器** 适用于告知其他"正在倾听的"蓝图已发生事件。事件发生时，正在倾听的蓝图便会作出反应，并相互独立地执行预期的操作。

例如，游戏中有一个 Boss，它被消灭时将会调用"OnDied"事件分配器。可将"OnDied"事件 [绑定](../event-dispatchers/binding-and-unbinding-events/index.md)在任意数量的其他蓝图中，如角色（作出庆祝动作）、关卡中的门（会打开）或 HUD（显示 UI 信息）。所有这些蓝图均在收到 Boss 被消灭的"OnDied"通知后执行。

> [!NOTE]
> 可在[事件分发器](../event-dispatchers/index.md)中查阅详细内容。
...
> [!NOTE]
> 可在 [在蓝图中实现接口](../types-of-blueprints/blueprint-interface/implementing-blueprint-interfaces/index.md)中查阅详细内容。
...
- 数个蓝图中存在一些相似功能，但在调用后执行不同的效果。
  ...
  - 玩家拥有一个火焰喷射器，使用时将触发一个名为

    ElementalDamage

    的事件。树木蓝图接受 ElementalDamage 调用并将树木烧毁，而雪人蓝图将把雪人融化。
