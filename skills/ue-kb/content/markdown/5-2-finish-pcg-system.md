...
- 围绕“5 2 Finish PCG System”整理 PCG 视频中的输入数据、图表规则、关键节点、参数风险和最终生成结果。
...
- `PCG`、`Mesh`、`Spline`、`Transform`、`Point`、`Spawn`、`Density`、`Random`、`Graph`、`Material`、`Instance`、`Landscape`、`more`、`tree`、`trees`、`PCG Graph`、`Surface Sampler`、`Transform Points`、`Filter`

## 分段知识

### 00:00:00-00:04:17 Spline 采样与路径生成
...
- 核对对象：`PCG`、`Spline`、`Surface Sampler`、`Material`、`采样`、`生成`。
...
- 核对对象：`Point`、`Density`、`Surface Sampler`、`Transform Points`、`Filter`、`Random`、`Material`、`Instance`、`属性`、`过滤`。
...
### 00:09:12-00:12:59 Spline 采样与路径生成
...
- 复现要点：属性命名要稳定，过滤、分区和分支节点应保留可检查的中间数据，避免后续规则难以追踪。
- 复现要点：Spline 流程要单独核对采样间距、切线、端点、交叉点和生成网格的对齐方式。
- 核对对象：`PCG Graph`、`PCG`、`Point`、`Density`、`Spline`、`Transform Points`、`Filter`、`采样`、`生成`。
