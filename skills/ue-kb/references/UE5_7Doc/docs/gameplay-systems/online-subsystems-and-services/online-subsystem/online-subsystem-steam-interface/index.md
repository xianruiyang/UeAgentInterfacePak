---
title: "Steam在线子系统"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/online-subsystem-steam-interface-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "Gameplay系统", "在线子系统和服务", "在线子系统", "Steam在线子系统"]
---

# Steam在线子系统

> 路径：虚幻引擎5.7文档 / Gameplay系统 / 在线子系统和服务 / 在线子系统 / Steam在线子系统

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/online-subsystem-steam-interface-in-unreal-engine
...
**Steam在线子系统API** 使你可以将虚幻引擎（UE）应用程序发布到[Valve的Steam平台](https://partner.steamgames.com/)。 **Steam** 模块的主要目的是帮助你通过一组功能（如媒介和通栏广告）将应用程序分发给Steam用户。 此外，Steam模块实现多个由[Online Subsystem](../index.md)公开的接口，支持Steamworks Software Development Kit（SDK）提供的大多数功能。
...
> [!NOTE]
> 请参考[Steam在线子系统API参考](https://dev.epicgames.com/documentation/unreal-engine/API/Plugins/OnlineSubsystemSteam/FOnlineSubsystemSteam)以了解当前可用的Steam接口的更完整列表。
...
>
> ![undefined](../../../../../assets/images/9b/9b6ba269c5d0848e2c4915b776ab43590659d8b354aec8bfbf7a7e344b2bfd72.jpg)
>
> 点击查看大图
...
| UE宏 | Steam命名 | 说明 |
| --- | --- | --- |
| `UE_PROJECT_STEAMPRODUCTNAME` | `STEAMPRODUCTNAME` | Valve建议将此设为版本ID，通常使用AppID（字符串形式）或无额外符号的简略产品名。 更改此字段（如更新版本ID）将导致Steam在匹配时忽略使用旧命名的所有活跃服务器，且需在合作伙伴后端修改工具信息。 |
| `UE_PROJECT_STEAMGAMEDIR` | `STEAMGAMEDIR` | 此通常为游戏的文件夹命名，且无可包含空格或符号。若该命名未包含空格或符号，则无需将其设为文件夹命名。 |
| `UE_PROJECT_STEAMGAMEDESC` | `STEAMGAMEDESC` | Valve建议将此宏设为产品全名。 |
| `UE_PROJECT_STEAMSHIPPINGID` | N/A | 要以UE 4.22和以上版本，在在Steam以外启动，此宏须包含产品的Steam ID。 |
...
确保将虚幻引擎Steam模块作为项目的一部分包含进去（请参阅[UnrealBuildTool目标文件](../../../../production-pipeline/using-the-unreal-engine-build-pipeline/unreal-build-tool/unreal-engine-build-tool-target-reference/index.md)以获取更多帮助）。 具体而言，在`mygame.build.cs`的构造函数中添加以下行应足以确保随着游戏构建Steam模块。
