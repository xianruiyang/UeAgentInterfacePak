---
title: "流送优化指南"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/stream-tuning-guide"
breadcrumbs: ["虚幻引擎5.7文档", "分享和发布项目", "像素流送", "像素流送开发指南", "流送优化指南"]
---

# 流送优化指南

> 路径：虚幻引擎5.7文档 / 分享和发布项目 / 像素流送 / 像素流送开发指南 / 流送优化指南

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/stream-tuning-guide
...
![MaxQuality对于图像质量和码率的影响](../../../../../assets/images/9e/9e1c16ca4c648ce3e4c16a60a9b69d03ec2da4bea2df787bf0c4e565420763b7.jpg)
...
| 延迟因素 | 指南 |
| --- | --- |
| 选用的视频编码器。 | VP8/VP9基于软件的编码器会比硬件加速的H.264编码器带来更多的延迟。 |
...
| 弹性因素 | 指南 |
| --- | --- |
| 编码器关键帧间隔 | 通过发送关键帧，流和解码器就可以在大量数据丢失后恢复。发送关键帧的间隔可以使用 -PixelStreaming.Encoder.KeyframeInterval` 控制。 关键帧确实会比普通帧占用更多的带宽，因此如果发生丢包的原因是网络饱和，发送更多关键帧可能无济于事。 |
| 编码器帧内刷新 | 视频流恢复信息可以在多个帧切片之间编码。利用此信息，流在发生数据丢失的情况下会更具弹性；但是，这确实会为整个流占用更多带宽，并在发生流恢复时引入扫描线类型的瑕疵。 目前此选项仅在NVIDIA GPU上可用，你可以使用 -NVENCIntraRefreshPeriodFrames=N`和`-NVENCIntraRefreshCountFrames=M` 在像素流送中启用它（其中N是再次发送帧内刷新之前的帧数，M是要使用帧内刷新数据编码的帧数。Pixel Streaming 2已移除了该选项）。 |
...
- 如果你能够接受更多的延迟并且像素流送应用不计划支持多用户像素流送，那么你可以尝试使用VP8/VP9软件编码器，使用

  -PixelStreamingEncoderCodec=

  。该编码器在同样的码率下能够比H.264生成质量更好的编码。
...
一对一用例是最简单、最常见的像素流送形式，即一个用户连接到一个像素流实例。有关像素流送配置的更多细节，请参阅[托管和网络指南](../hosting-and-networking-guide-for-pixel-streaming/index.md)
...
- 对于无法满足这些要求的用户，可以使用TURN（中继）服务器，但其延迟

  将

  比P2P用户更糟糕。
- 当多个WebRTC对等体需要以不同的质量查看同一流送时，此配置
...
- 确保协商要约/答复包含
  ...
  （例如Chrome、Firefox），并使用-PixelStreamingWebRTCEnableFlexFec。然而，请注意这样将采用更多的比特率。
...
- 1.5-10兆比特/秒：
  ...
  在低端，随着网络波动，用户可能会体验到一些明显的视觉质量变化（例如，运动过程中的像素化在视频静止时会变得清晰）；
- 0.5-1.5兆比特/秒：
