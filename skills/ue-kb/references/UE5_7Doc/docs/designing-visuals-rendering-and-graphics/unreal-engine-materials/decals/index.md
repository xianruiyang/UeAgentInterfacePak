---
title: "贴花材质"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/decal-materials-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "设计视觉、渲染和图形效果", "材质", "贴花材质"]
---

# 贴花材质

> 路径：虚幻引擎5.7文档 / 设计视觉、渲染和图形效果 / 材质 / 贴花材质

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/decal-materials-in-unreal-engine
...
![石墙上的贴花示例](../../../../assets/images/59/59466e0c6da4be19aa6be7fae8d7d69459b85c5cd06371bfefc94a5827726ed1.png)
...
- 在
  ...
  中累积所有贴花，它会存储BasePass之前的基础颜色、法线和粗糙度信息。然后在BasePass材质中对DBuffer取样。
- 在BasePass之后，但在光照之前，直接将贴花混合到
...
> [!WARNING]
> Substrate和DBuffer材质表达式需要启用DBuffer贴花才能正确渲染。如需更多信息和上下文，请参阅[DBuffer贴花平台和功能支持](#dbuffer%E8%B4%B4%E8%8A%B1%E5%B9%B3%E5%8F%B0%E5%92%8C%E5%8A%9F%E8%83%BD%E6%94%AF%E6%8C%81)。
...
![贴花材质属性](../../../../assets/images/99/99286d8775efcff7db4012f5b6911d8cc74de7abd48a11f25b9a33de0d5c5543.png)
...
> [!NOTE]
> DBuffer贴花不支持[调制](../unreal-engine-material-properties/material-blend-modes/index.md#%E8%B0%83%E5%88%B6)混合模式，但这也不会导致编译错误。如果选择“调制”，材质会成功编译，但混合模式会静默回退为半透明混合模式。

> [!TIP]
> 如果你的材质需要更多混合选项，请参阅本页面的[DBuffer材质表达式](#dbuffer%E6%9D%90%E8%B4%A8%E8%A1%A8%E8%BE%BE%E5%BC%8F)章节。
...
![创建贴花Actor](../../../../assets/images/e0/e0b5096440d739b6def2c051d76dd611efe446c63a8cc71bda61c2722d1a0784.jpg)

贴花Actor是一个与方向相关的线框盒体，决定了贴花材质投射的边界。其箭头指示贴花的投射方向。

![贴花Actor线框](../../../../assets/images/b9/b9903d66a1f2be62aad0c3e860b3c90d6921a5df4e0ca67007b911c6b9fa1116.jpg)
...
> 图片已省略：没有和有旋转消退的贴花Actor边界框。

贴花Actor具有以下属性：

![贴花Actor属性](../../../../assets/images/27/274aa4dc0bff19584df5698d24d1595745e896ab48db9002570714be4565eeb7.jpg)
...
| **贴花颜色（Decal Color）** | 一种设置贴花[基础颜色](../material-inputs/index.md#%E5%9F%BA%E7%A1%80%E9%A2%9C%E8%89%B2)的方式。你可以使用Decal Color节点在材质图表中访问此颜色。 |
...
![旋转的贴花Actor](../../../../assets/images/be/bed7ae54d2e34a49a8259f5a73e8bfaa21dfb724759940573e38943056cd1d58.png)
...
![接收贴花设置](../../../../assets/images/61/61d4c36d1c20c6b484bbf50fd6fec43c82fa4980d1b69031cd2d61c2c31d3547.png)

下面的示例展现了与贴花Actor相交的紧密Actor群集。只有墙壁受投射的贴花材质影响。其他两个对象都禁用了“接收贴花”，所以不受贴花Actor的体积影响。

![接收贴花示例](../../../../assets/images/3f/3f80b7db4ae164f52ece397eb9f309c7c2d3db0ea59f2e631cf6d0e03b5ae2b6.jpg)
...
利用这些DBuffer材质表达式，可以访问可构成[贴花响应](#%E8%B4%B4%E8%8A%B1%E5%93%8D%E5%BA%94)的三种DBuffer纹理中的每一个 — BaseColor、WorldNormal和Roughness。这些节点将DBuffer中的纹理数据直接读取到材质图表中，为你的贴花材质带来可自定义的灵活性，例如近似重现虚幻引擎4（UE4）中的旧版染色贴花行为。
...
若要优化DBuffer贴花的性能，最好仅为实际需要接收贴花的材质启用DBuffer贴花响应，这包括在接收材质中启用实际使用的通道（具体请参阅[贴花响应](#%E8%B4%B4%E8%8A%B1%E5%93%8D%E5%BA%94)一节）。凡是利用DBuffer的材质，都有额外代码来执行此操作，这进而增加了材质的复杂性。

重新投射上一帧的法线也可能会带来一些开销，但仅限于受贴花影响的像素的取样和重新投射。

在使用路径追踪器渲染时，[网格体贴花](#%E7%BD%91%E6%A0%BC%E4%BD%93%E8%B4%B4%E8%8A%B1)会产生较大的开销，因为维护和追踪贴花的另一个加速结构（即TLAS）会产生费用，并且在追踪TLAS之前对贴花投射排序也有额外的开销。一般来说，贴花的屏幕覆盖范围越大，贴花的开销就越大。
...
| 列 1 | 列 2 |
| --- | --- |
| [undefined](https://d1iv7db44yhgxn.cloudfront.net/documentation/images/e15759d2-8398-4db4-89a0-20bca958c0c3/origin-pathtracer-01.png) | [undefined](https://d1iv7db44yhgxn.cloudfront.net/documentation/images/7ba6c384-69dd-4e3e-b311-c071c57cc1ef/origin-pathtracer-02.png) |
...
虚幻引擎5项目会默认启用DBuffer贴花，但你仍可以在一些不受支持的平台（例如移动平台）上强制关闭它。这可能会影响已使用[Dbuffer材质表达式](#dbuffer%E6%9D%90%E8%B4%A8%E8%A1%A8%E8%BE%BE%E5%BC%8F)创作的材质。这还可能为需要启用DBuffer贴花的功能带来问题，例如[Substrate](https://dev.epicgames.com/documentation/404)。

如果你的项目将支持多个平台，你需要在创作材质时牢记这一点。一种做法是使用 **Feature Level Switch** 材质节点，简化更低端渲染路径的材质图表，而不必为这些路径使用单独的材质。

### DBuffer贴花的移动兼容性

移动平台不支持DBuffer贴花渲染。移动正向着色（OpenGL ES 3.2、Metal、Vulkan和Nintento Switch）也不支持GBuffer贴花渲染，而是通过将基础颜色（BaseColor）和自发光应用到光照场景颜色来模拟贴花。使用[移动延迟着色](../../../mobile-development/rendering-features-for-mobile-games/using-the-mobile-deferred-shading-mode/index.md)时，会使用GBuffer贴花，但是不支持环境光遮蔽。

如需详细了解移动渲染路径的贴花支持，请参阅[移动渲染功能](../../../mobile-development/rendering-features-for-mobile-games/index.md)。

#### Substrate贴花的移动兼容性

如果你在项目中使用了[Substrate](https://dev.epicgames.com/documentation/404)，移动延迟着色将不会使用GBuffer，并且仅支持渲染DBuffer贴花。
...
> 图片已省略：间接点亮的GBuffer和DBuffer贴花示例
...
正如[重现旧版行为](#%E9%87%8D%E7%8E%B0%E6%97%A7%E7%89%88%E8%A1%8C%E4%B8%BA)一节中的示例中所提到的那样，你可以使用[DBuffer材质表达式](#dbuffer%E6%9D%90%E8%B4%A8%E8%A1%A8%E8%BE%BE%E5%BC%8F)重现虚幻引擎4中的染色贴花混合模式的外观和应用，但不能完全重现，因为没有考虑重叠贴花。
