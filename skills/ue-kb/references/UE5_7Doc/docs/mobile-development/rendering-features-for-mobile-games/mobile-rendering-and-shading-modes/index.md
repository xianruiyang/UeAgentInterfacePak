---
title: "渲染和着色模式"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/mobile-rendering-and-shading-modes-for-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "移动端开发", "移动端渲染功能", "渲染和着色模式"]
---

# 渲染和着色模式

> 路径：虚幻引擎5.7文档 / 移动端开发 / 移动端渲染功能 / 渲染和着色模式

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/mobile-rendering-and-shading-modes-for-unreal-engine

UE提供了适合在移动设备上使用的 **前向** 和 **延迟** 着色模式，以及对 **桌面渲染器** 的试验性支持。每种着色模式在性能和保真度方面均有不同的权衡。本页面概述了移动端的前向和桌面着色模式，而桌面渲染器则在[单独的指南](https://dev.epicgames.com/documentation/404)中进行介绍。
...
| **值** | **着色模式** | **说明** |
| --- | --- | --- |
| 0 | 移动前向着色（Mobile Forward Shading） | 在运行时处理光照和阴影。适用于使用预计算光照的项目。 |
| 1 | 移动延迟着色（Mobile Deferred Shading） | 处理光照和阴影 |

或者，你可以在高端移动设备上使用 **桌面渲染器** ，但请记住，这会产生显著的性能开销。有关更多详情，请参阅[移动端桌面渲染器](https://dev.epicgames.com/documentation/404)。
...
**延迟着色** 将几何体和光照/着色作为单独的通道进行处理，而不是在运行时同时处理。为高质量反射、多个动态光源、光照贴花和其他高级光照功能提供支持，这些功能在移动端的默认前向渲染模式中通常不可用。有关其保真度和性能改进的更多详细信息，请参阅[移动延迟着色](../using-the-mobile-deferred-shading-mode/index.md)。
...
有关更多细节，请参阅[移动延迟着色](../using-the-mobile-deferred-shading-mode/index.md)。
