---
title: "自动换行框"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/wrapping-umg-widgets-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "创建用户界面", "控件类型参考说明", "自动换行框"]
---

# 自动换行框

> 路径：虚幻引擎5.7文档 / 创建用户界面 / 控件类型参考说明 / 自动换行框

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/wrapping-umg-widgets-in-unreal-engine
...
| **当小于设定值时填充跨度（Fill Span when Less Than）** | 该设置表示如果自动换行框中的可用空间总值降到指定阈值之下，则槽将尝试填充整行。将阈值设为 0 表示不会进行填充。 |
| **水平对齐（Horizontal Alignment）** | 水平对齐对象。 |
| **垂直对齐（Vertical Alignment）** | 垂直对齐对象。 |
| **强制换行（Force New Line）** | 将当前槽设置为新一行的开头。 |
