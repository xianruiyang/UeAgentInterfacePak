# Learn the Art of Exterior PCG Detailing ｜ Building Series Part 6

# Learn the Art of Exterior PCG Detailing ｜ Building Series Part 6

## 知识目标

- 围绕“Learn the Art of Exterior PCG Detailing ｜ Building Series Part 6”整理 PCG 视频中的输入数据、图表规则、关键节点、参数风险和最终生成结果。
...
- `PCG`、`Blueprint`、`Static Mesh`、`Mesh`、`Spline`、`Transform`、`Point`、`Actor`、`Spawn`、`Density`、`Random`、`Seed`、`Loop`、`Graph`、`floor`、`unique`、`PCG Graph`、`Point Data`、`Spline Sampler`、`Static Mesh Spawner`、`Transform Points`、`Copy Points`、`Filter`、`Branch`
...
- 核对对象：`PCG Graph`、`PCG`、`Point Data`、`Point`、`Actor`、`Blueprint`、`Loop`、`Random`、`生成`、`蓝图`。
...
- 核对对象：`PCG`、`Point Data`、`Point`、`Density`、`Filter`、`Loop`、`属性`、`过滤`。
...
### 00:09:15-00:14:08 Geometry Script 与 Dynamic Mesh 处理
...
- 核对对象：`PCG`、`Point`、`Density`、`Actor`、`Blueprint`、`Filter`、`Branch`、`Loop`、`Geometry Script`、`Dynamic Mesh`。
...
### 00:14:08-00:19:00 Geometry Script 与 Dynamic Mesh 处理
...
- 复现要点：属性命名要稳定，过滤、分区和分支节点应保留可检查的中间数据，避免后续规则难以追踪。
- 复现要点：Dynamic Mesh/Geometry Script 节点通常计算成本较高，布尔、倒角和 Auto UV 应在质量与重算成本之间取舍。
- 核对对象：`PCG`、`Point`、`Density`、`Filter`、`Random`、`Geometry Script`、`Dynamic Mesh`、`Boolean`。
...
- 复现要点：先用 Debug/Inspect 核对点数量、Bounds、Density 和关键属性，再判断最终生成结果。
- 复现要点：属性命名要稳定，过滤、分区和分支节点应保留可检查的中间数据，避免后续规则难以追踪。
- 核对对象：`PCG`、`Point`、`Density`、`Branch`、`Loop`、`属性`、`过滤`。
...
### 00:22:34-00:27:27 Geometry Script 与 Dynamic Mesh 处理
...
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 复现要点：Dynamic Mesh/Geometry Script 节点通常计算成本较高，布尔、倒角和 Auto UV 应在质量与重算成本之间取舍。
- 核对对象：`Point`、`Density`、`Blueprint`、`Static Mesh`、`Static Mesh Spawner`、`Transform Points`、`Filter`、`Random`、`Geometry Script`、`Dynamic Mesh`。
...
### 00:27:27-00:32:23 Geometry Script 与 Dynamic Mesh 处理
...
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 复现要点：Dynamic Mesh/Geometry Script 节点通常计算成本较高，布尔、倒角和 Auto UV 应在质量与重算成本之间取舍。
- 核对对象：`PCG Graph`、`PCG`、`Point`、`Actor`、`Blueprint`、`Static Mesh`、`Static Mesh Spawner`、`Transform Points`、`Loop`、`Seed`。
...
### 00:32:24-00:34:22 Spline 采样与路径生成
...
- 核对对象：`Point`、`Spline`、`Spline Sampler`、`Actor`、`Copy Points`、`采样`、`生成`。
