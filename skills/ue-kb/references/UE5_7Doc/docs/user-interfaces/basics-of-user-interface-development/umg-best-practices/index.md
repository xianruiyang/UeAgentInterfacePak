---
title: "UMG 最佳实践"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/umg-best-practices-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "创建用户界面", "构建用户界面", "UMG 最佳实践"]
---

# UMG 最佳实践

> 路径：虚幻引擎5.7文档 / 创建用户界面 / 构建用户界面 / UMG 最佳实践

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/umg-best-practices-in-unreal-engine

使用 **虚幻示意图形（UMG）** 进行实验并创建 UI 元素时，以最高效的方式进行操作无疑是您的目标。 虽然这较大程度上取决于您项目的规模，但您在设置 UI 时也需要考虑到本页中提及的要点。

例如，创建的简单 UI 画面包含少量变化频率较高的变量时，则需要考虑使用 [属性绑定](../../umg-editor-reference/property-binding-for-umg/index.md) 来驱动这些变更。 然而，如果 UI 画面更为复杂，数个属性将在特定时间发生变更，则使用 [基于事件的](../../tutorials-and-examples-for-user-interfaces/driving-ui-updates-with-events/index.md) 逻辑执行这些变更为佳，而无需在每帧进行检查确认是否应该形成变更。
...
可预览在其他屏幕尺寸上的效果，但不要在不同屏幕尺寸上构建画面，因为这会导致内容之间的比例不一致。 谈到比例，在 UMG 中创建内容时，尝试以 [DPI 缩放](../../umg-editor-reference/dpi-scaling/index.md) 1.0 进行创建。
...
> [!NOTE]
> 可在 [移动端渲染功能](../../../mobile-development/rendering-features-for-mobile-games/index.md) 中查阅移动设备纹理创建的更多内容。
...
使用如垂直框或横向框的面板时，需要确定是否对 [槽](../building-your-ui/umg-slots/index.md) 尺寸使用 **填充（Fill）** 或 **自动（Auto）**。
