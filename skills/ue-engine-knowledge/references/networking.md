# Networking / Multiplayer 知识卡

## 适用范围

多人权威模型、Replication、RPC、Ownership、Prediction、GameMode/GameState 和 PlayerState。
...
- Server 拥有权威状态，Client 负责输入和表现预测。
- GameMode 只在 server 存在。
- RPC 必须检查方向、owner 和可靠性。
- 高频状态优先属性复制、压缩和条件复制。
