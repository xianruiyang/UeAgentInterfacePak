# Easily Create a Modular Maze Using PCG a

# Easily Create a Modular Maze Using PCG a

## 知识目标

- 围绕“Easily Create a Modular Maze Using PCG a”整理 PCG 视频中的输入数据、图表规则、关键节点、参数风险和最终生成结果。
...
- `PCG`、`Static Mesh`、`Mesh`、`Point Filter`、`Spline`、`Transform`、`Point`、`Attribute`、`Actor`、`Component`、`Spawn`、`Grid`、`Bounds`、`Density`、`Random`、`Seed`、`Loop`、`Graph`、`PCG Graph`、`PCG Component`、`Point Data`、`Spline Sampler`、`Volume`、`Static Mesh Spawner`

## 分段知识

### 00:00:00-00:04:58 属性、过滤与数据分流
...
- 核对对象：`PCG Graph`、`PCG`、`Point`、`Actor`、`Static Mesh`、`Partition`、`Runtime Generation`、`Hierarchical Generation`、`Material`、`Instance`。
...
- 复现要点：属性命名要稳定，过滤、分区和分支节点应保留可检查的中间数据，避免后续规则难以追踪。
- 复现要点：运行时、分区或 GPU 生成需要单独验证缓存、触发时机、平台支持和性能预算。
- 核对对象：`PCG Graph`、`PCG Component`、`PCG`、`Point`、`Bounds`、`Volume`、`Actor`、`Partition`、`Hierarchical Generation`、`Data Asset`。
...
- 复现要点：属性命名要稳定，过滤、分区和分支节点应保留可检查的中间数据，避免后续规则难以追踪。
- 复现要点：运行时、分区或 GPU 生成需要单独验证缓存、触发时机、平台支持和性能预算。
- 核对对象：`PCG`、`Point Data`、`Point`、`Attribute`、`Density`、`Partition`、`Random`、`Data Asset`、`Instance`、`属性`。
...
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 复现要点：运行时、分区或 GPU 生成需要单独验证缓存、触发时机、平台支持和性能预算。
- 核对对象：`PCG Graph`、`PCG`、`Point`、`Attribute`、`Static Mesh`、`Static Mesh Spawner`、`Copy Points`、`Partition`、`Loop`、`Random`。
...
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 复现要点：运行时、分区或 GPU 生成需要单独验证缓存、触发时机、平台支持和性能预算。
- 核对对象：`Point`、`Attribute`、`Static Mesh`、`Transform Points`、`Partition`、`属性`、`过滤`。
...
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 复现要点：运行时、分区或 GPU 生成需要单独验证缓存、触发时机、平台支持和性能预算。
- 核对对象：`Point`、`Attribute`、`Bounds`、`Actor`、`Static Mesh`、`Static Mesh Spawner`、`Transform Points`、`Filter`、`Partition`、`生成`。
...
### 00:28:40-00:33:38 Spline 采样与路径生成
...
- 复现要点：运行时、分区或 GPU 生成需要单独验证缓存、触发时机、平台支持和性能预算。
- 核对对象：`Point`、`Attribute`、`Density`、`Bounds`、`Spline`、`Spline Sampler`、`Filter`、`Partition`、`Random`、`采样`。
...
- 复现要点：属性命名要稳定，过滤、分区和分支节点应保留可检查的中间数据，避免后续规则难以追踪。
- 复现要点：运行时、分区或 GPU 生成需要单独验证缓存、触发时机、平台支持和性能预算。
- 核对对象：`PCG`、`Point`、`Attribute`、`Partition`、`Loop`、`Data Asset`、`Instance`、`属性`、`过滤`。
