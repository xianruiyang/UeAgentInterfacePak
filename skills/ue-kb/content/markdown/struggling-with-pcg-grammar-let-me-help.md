# Struggling With PCG Grammar Let Me Help

# Struggling With PCG Grammar Let Me Help

## 知识目标

- 围绕“Struggling With PCG Grammar Let Me Help”整理 PCG 视频中的输入数据、图表规则、关键节点、参数风险和最终生成结果。
...
- `PCG`、`Blueprint`、`Static Mesh`、`Mesh`、`Spline`、`Transform`、`Point`、`Attribute`、`Actor`、`Component`、`Spawn`、`Grid`、`Bounds`、`Density`、`Graph`、`Material`、`height`、`PCG Graph`、`PCG Component`、`Static Mesh Spawner`、`Branch`、`Boolean`、`Data Asset`、`Data Table`
...
- 核对对象：`PCG Graph`、`PCG Component`、`PCG`、`Point`、`Spline`、`Actor`、`Blueprint`、`采样`、`生成`。
...
### 00:03:13-00:08:00 Spline 采样与路径生成
...
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 复现要点：Spline 流程要单独核对采样间距、切线、端点、交叉点和生成网格的对齐方式。
- 核对对象：`PCG Graph`、`PCG`、`Point`、`Attribute`、`Bounds`、`Spline`、`Static Mesh`、`Static Mesh Spawner`、`采样`、`生成`。
...
### 00:08:00-00:11:21 Spline 采样与路径生成
...
- 复现要点：属性命名要稳定，过滤、分区和分支节点应保留可检查的中间数据，避免后续规则难以追踪。
- 复现要点：Spline 流程要单独核对采样间距、切线、端点、交叉点和生成网格的对齐方式。
- 核对对象：`Point`、`Attribute`、`Bounds`、`Spline`、`采样`、`生成`。
...
### 00:11:22-00:14:18 Spline 采样与路径生成
...
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 复现要点：Spline 流程要单独核对采样间距、切线、端点、交叉点和生成网格的对齐方式。
- 核对对象：`PCG`、`Attribute`、`Bounds`、`Spline`、`Static Mesh`、`Static Mesh Spawner`、`Material`、`采样`、`生成`。
...
### 00:14:18-00:17:25 Spline 采样与路径生成
...
- 复现要点：属性命名要稳定，过滤、分区和分支节点应保留可检查的中间数据，避免后续规则难以追踪。
- 复现要点：Spline 流程要单独核对采样间距、切线、端点、交叉点和生成网格的对齐方式。
- 核对对象：`PCG`、`Point`、`Attribute`、`Spline`、`Data Asset`、`Data Table`、`采样`、`生成`。
...
- 复现要点：属性命名要稳定，过滤、分区和分支节点应保留可检查的中间数据，避免后续规则难以追踪。
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 核对对象：`Point`、`Attribute`、`Static Mesh`、`Static Mesh Spawner`、`生成`、`蓝图`。
...
### 00:20:23-00:25:20 Geometry Script 与 Dynamic Mesh 处理
...
- 核对对象：`Attribute`、`Spline`、`Branch`、`Geometry Script`、`Dynamic Mesh`、`Boolean`。
...
### 00:25:20-00:30:14 Spline 采样与路径生成
...
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 复现要点：Spline 流程要单独核对采样间距、切线、端点、交叉点和生成网格的对齐方式。
- 核对对象：`Point`、`Attribute`、`Spline`、`Static Mesh`、`Static Mesh Spawner`、`采样`、`生成`。
