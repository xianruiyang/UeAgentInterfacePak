---
title: "屏幕百分比与时序上采样"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/screen-percentage-with-temporal-upscale-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "设计视觉、渲染和图形效果", "优化和调试实时渲染项目", "屏幕百分比与时序上采样"]
---

# 屏幕百分比与时序上采样

> 路径：虚幻引擎5.7文档 / 设计视觉、渲染和图形效果 / 优化和调试实时渲染项目 / 屏幕百分比与时序上采样

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/screen-percentage-with-temporal-upscale-in-unreal-engine
...
- 主要空间放大（Primary Spatial Upscaling）

  就是以前使用的屏幕百分比功能。它的基本理念是以较低的分辨率渲染画面，然后先放大，再绘制用户界面（UI）。
- 次要空间放大（Secondary Spatial Upscaling）

  执行第二次也是最终的空间放大传递，与主要放大传递无关。
...
![NoUpscaling.png](../../../../assets/images/18/184eb514a5aea82e6dd511c42c0eeac517c794f0634f62feb9c6eba8134fe956.png)

对于每个GPU帧，所有渲染目标在管道中自始至终使用其全分辨率。

![SpatialUpscale.png](../../../../assets/images/5e/5efc4b5b913ccd7ca6cfae19c7adfa10d5e52ec17e858b22b1fdc58d1275fb6f.png)
...
![SpatialAndTemporalUpsample.png](../../../../assets/images/62/62058e4f15fb66d85b23022b7743e0b7f6990bfba6baa5e539cd0e3decc5adfb.png)
...
![屏幕百分比：70 | 时序上采样：禁用](../../../../assets/images/2c/2c36b8c09968e1cd333fc9b4403f47522e0f6295776d08fc42de13bdbcc2a074.jpg)

![屏幕百分比：70 | 时序上采样：启用](../../../../assets/images/b8/b8237dfcfd37cd4002adfc077c7257c181e123eb4785145cab893573edbbde6e.jpg)
...
![屏幕百分比：70 | 时序上采样：禁用](../../../../assets/images/60/602efe3d2db0668e4c56483b16538f2b6a550b4a74039a683b617c9cc6f44b61.jpg)

![屏幕百分比：70 | 时序上采样：启用](../../../../assets/images/c4/c46795b7513cb79e0b953fafd640b734231e567b9e44226a1df08188db1fbf57.jpg)
...
> 图片已省略：AutomaticViewMipBias.png
...
> 图片已省略：AutomaticViewMipBiasUnchecked.png
...
| 100% | 200% | 如果要使[动态分辨率](../dynamic-resolution/index.md)在不被用于渲染目标的GPU内存限制的前提下具有超过100%的理论可能，那么这是理想选择。 |
...
> 图片已省略：ViewProperty.png

可以像这样根据像素位置重新计算视口UV：

> 图片已省略：ScreenPosFromPixelPos.png

既然Scene Texture表达式只接受视口UV，就可以在执行UV视口转换的同时在任意场景纹理中取样，而且不可能出错。

## 次要空间放大

除了[主要空间放大](#primaryscreenpercentage)外，还有 **次要空间放大（Secondary Spatial Upscale）**，它发生在[TAAU](#temporalanti-aliasingupsample)传递之后。对于高DPI的显示器这可能产生问题，因为GPU可能无法适应在较高分辨率下渲染的场景复杂度。为此，渲染器支持使用驱动第二和最终放大传递的次要屏幕百分比。此传递与主要屏幕百分比无关。

> [!NOTE]
> 请注意，与主要空间放大不同的是，次要空间放大是静态的，无法在游戏中动态更改。

> 图片已省略：PrimaryAndSecondaryScreenPercentageUpscale.png
...
```
	目标分辨率 / 当前设置的分辨率 * 100 = 次要屏幕百分比	900 / 1080 * 100 = 0.8333 * 100 = 83.33 
```
...
> 图片已省略：DPIEditorViewportScaling.png
