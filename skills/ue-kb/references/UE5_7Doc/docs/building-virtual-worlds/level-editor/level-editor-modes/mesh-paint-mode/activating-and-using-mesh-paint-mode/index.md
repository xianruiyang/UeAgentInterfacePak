---
title: "激活和使用网格体绘制模式"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/activating-and-using-mesh-paint-mode-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "构建虚拟世界", "关卡编辑器", "关卡编辑器模式", "网格体绘制模式", "激活和使用网格体绘制模式"]
---

# 激活和使用网格体绘制模式

> 路径：虚幻引擎5.7文档 / 构建虚拟世界 / 关卡编辑器 / 关卡编辑器模式 / 网格体绘制模式 / 激活和使用网格体绘制模式

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/activating-and-using-mesh-paint-mode-in-unreal-engine
...
> [!NOTE]
> 如需详细了解网格体绘制工具的各个属性和设置，请参阅[网格体绘制模式工具和设置](../mesh-paint-tool-reference/index.md)。
...
| --- | --- |
|  |  |
| 顶点颜色、纹理颜色和纹理的颜色绘制设置 | 顶点权重的权重绘制设置 |
...
| 属性 | 说明 |
| --- | --- |
| **纹理权重类型（Texture Weight Type）** | 选择要使用的混合权重绘制模式： **Alpha（双纹理）** ：使用Alpha值在两个纹理之间插值。 **RGB（三纹理）** ：根据颜色通道对三个纹理应用权重。 **ARGB（四纹理）** ：根据颜色通道和Alpha通道对四个纹理应用权重。 **ARGB - 1（五纹理）** ：根据颜色通道和Alpha通道对五个纹理应用权重。 |
...
> 动图已省略：65bd8d2296a9924998eb72dfa6b1084aefba51e1177762a3dd13d6ed6a6bbbc9
...
- Nanite网格体不支持顶点颜色绘制：
  ...
  - Nanite网格体应使用纹理颜色进行网格体绘制。这还要求项目启用

    虚拟纹理

    ，因为纹体理颜色的网格体绘制纹理始终是虚拟纹理。
...
- 在启用了Nanite后，即使存在顶点颜色数据，也无法逐实例可视化：
  ...
  - 要可视化顶点颜色数据，应在编辑器中暂时禁用Nanite。禁用后，将使用回退网格体来可视化此颜色数据。要暂时禁用Nanite，请执行以下操作：
    ...
    - 使用控制台将
      ...
      设置为0。将其设置回1以重新启用。
    - 在网格体组件的渲染（Rendering） > 高级（Advanced）设置下，勾选
