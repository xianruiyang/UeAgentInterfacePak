---
title: "支持多种媒体配置"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/supporting-multiple-media-configurations-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "使用媒体", "媒体集成", "专业视频I/O", "支持多种媒体配置"]
---

# 支持多种媒体配置

> 路径：虚幻引擎5.7文档 / 使用媒体 / 媒体集成 / 专业视频I/O / 支持多种媒体配置

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/supporting-multiple-media-configurations-in-unreal-engine

如果你已遵循[AJA媒体](https://dev.epicgames.com/documentation/404)或[Blackmagic Design](../blackmagic-video-io-quick-start/index.md)硬件的快速入门指南进行了操作，那么你已经看到了一种设置媒体输入和媒体采集资源的可能方法，可以将SDI视频源导入和导出你的项目。
...
![Media Profile Overview](../../../../../assets/images/5d/5d7b5bacfa917425af2457a2224b1b85480d3cbd12af0291967401959e644c46.jpg)
...
2. 当你安装了
   ...
   媒体框架工具插件（Media Framework Utilities Plugin）

   时，项目的

   项目设置（Project Settings）

   面板包含类似的输入和输出列表。媒体配置文件的输入列表中的每个编号条目都与项目设置的输入列表中的相同编号匹配，输出列表也是如此。
...
3. 然后，当需要在项目内容中设置媒体源或媒体输出时，你可以随时引用这些代理媒体资源。例如：
   ...
   2. 类似地，在设置媒体采集时，将输出发送到代理媒体输出，而不是直接发送到AJA或Blackmagic媒体输出。你可以在如上所示的虚幻编辑器中的

      媒体采集（Media Capture）

      面板中执行此操作，或者在运行时调用

      创建媒体采集（Create Media Capture）

      蓝图节点时执行此操作。如果你在项目设置中设置了输出列表，以包含对相同代理媒体输出的引用，那么采集将被转发到输出，并使用你在媒体配置文件中配置的匹配索引。
...
![Selecting the active Media Profile](../../../../../assets/images/36/3646981332d6931ac88eb9d2169a0dd2983c54bac04650b287fc25689faacd91.jpg)
...
![Startup Media Profile](../../../../../assets/images/a5/a5c3d1ece6892253970cd932891661fe04f06f78c05af1aebb8a5414b415983a.png)
...
> - 媒体配置文件和代理资源由
>
>   媒体框架工具（Media Framework Utilities）
>
>   插件提供。你将需要安装这个插件。
> - 你还需要遵循
>
...
> [!TIP]
> 要查看非常类似于下面描述的设置的工作示例，请参阅Epic Games启动器的学习（Learn）选项卡所提供的[**Virtual Studio展示**](../../../../samples-and-tutorials/engine-feature-examples/virtual-studio-sample-project/index.md)。

1. 首先创建代理媒体源资源。在 **内容浏览器（Content Browser）** 中右键单击，从上下文菜单中选择 **媒体（Media）> 代理媒体源（Proxy Media Source）**。

   ![Create a Proxy Media Source](../../../../../assets/images/d1/d13fa0addfa3eee60455bdc28ada32b1c3c6daa233f05ab34ee4b7afd21dae99.jpg)
...
3. 创建一个新的媒体束资源。在 **内容浏览器（Content Browser）** 中右键单击，选择 **媒体（Media）> 媒体束（Media Bundle）**。
   ...
   ![Create a Media Bundle](../../../../../assets/images/21/2160561403bf3760a507e1b725cc56b290ad15b491f71d1a0c29a1725f6af3cd.jpg)
...
5. 在 **详细信息（Details）** 面板中，找到 **媒体源（Media Source）** 设置，并从下拉列表中选择 **代理媒体源（Proxy Media Source）**。
   ...
   ![Select Proxy Media Source](../../../../../assets/images/8a/8aaab59afed2180f08d980d268ce237fc7154f315a8dbb4a94246b572890cd3f.png)

   点击查看大图
6. 展开 **源（Source）** 类别，并将 **代理（Proxy）** 设置为引用前面创建的 **VideoProxyInA** 资源。

   ![Set the Proxy Media Source Asset](../../../../../assets/images/17/17b5b8fa3abe81a8fa6918adb7127309745297fe993aee4ca19e2a5b3656ebba.png)
...
8. 从主菜单中，选择 **编辑（Edit）> 项目设置（Project Settings）**。找到 **插件（Plugins）> 媒体配置文件（Media Profile）** 部分，并展开高级选项。
   ...
   ![Media Profile settings](../../../../../assets/images/a7/a75ca132b1a9578f1af1cc0fc3137aeffd8c9f51779712c1ebd3ade993a51da9.png)

   点击查看大图
9. 向 **媒体源代理（Media Source Proxy）** 列表添加两个条目。将其中一个条目设置为引用 **VideoProxyInA**，另一个设置为引用 **VideoProxyInB**。

   > 图片已省略：Add the Proxy Media Source Assets
...
10. 现在，我们将创建一个名为
    ...
    的新媒体配置文件，它可以从磁盘上的文件中播放视频。有两种方法可以做到这一点：

    - 在 **内容浏览器（Content Browser）** 中右键单击，从上下文菜单中选择 **媒体（Media）> 媒体配置文件（Media Profile）**，然后重命名资源。

      > 图片已省略：Right-click to create the new Asset
...
12. 重复前面的步骤以创建第二个媒体配置文件，名为 **LiveFeedProfile**，它从连接你计算机的AJA或Blackmagic设备导入视频。例如：
    ...
    > 图片已省略：Media Profile for live feeds

    点击查看大图

    各个媒体配置文件还提供了设置时间码提供者和集中同步提供者的功能：

    > 图片已省略：媒体配置文件中的时间码和集中同步设置

    它们与 **项目设置（Project Settings）** 面板中提供的 **时间码提供者（TimecodeProvider）** 和 **自定义时间步（Custom TimeStep）** 设置具有完全相同的效果。但是，当你在媒体配置文件中设置这些值时，只有当该媒体配置文件处于活动状态时，它们才会覆盖 **项目设置（Project Settings）**。详情请参阅[时间码和集中同步](../timecode-and-genlock/index.md)。
13. 使用工具栏中的配置文件选择按钮选择上面创建的 **FileMediaProfile**。

    > 图片已省略：激活FileMediaProfile
14. 将你的两个媒体束拖放到关卡视口中。你应该看到它们开始显示从文件加载的视频或通过SDI连接传入的视频。

    > 图片已省略：Drag and drop the Media Bundles
...
1. 首先创建一个代理媒体输出资源。在 **内容浏览器（Content Browser）** 中右键单击，从上下文菜单中选择 **媒体（Media）> 代理媒体输出（Proxy Media Output）**。
   ...
   > 图片已省略：Create a new Proxy Media Output
...
8. 要在运行时采集视频源并通过代理发送它，请完全遵循[AJA媒体](../aja-video-io-quick-start/index.md)或[Blackmagic Design](../blackmagic-video-io-quick-start/index.md)硬件的快速入门指南进行操作。 唯一的区别是，你使用要将采集发送到的 **ProxyMediaOutput** 的对象引用，而不是使用一个作为 **AjaMediaOutput** 或 **BlackmagicMediaOutput** 的对象引用变量。然后使用这个 **ProxyMediaOutput** 变量作为 **创建媒体采集（Create Media Capture）** 节点的输入。例如：

   > 图片已省略：Create the Media Capture from the Proxy Media Output
