---
title: "XR最佳实践"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/forward-shading-renderer-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "设计视觉、渲染和图形效果", "优化和调试实时渲染项目", "XR最佳实践"]
---

# XR最佳实践

> 路径：虚幻引擎5.7文档 / 设计视觉、渲染和图形效果 / 优化和调试实时渲染项目 / XR最佳实践

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/forward-shading-renderer-in-unreal-engine
...
| **r.HZBOcclusion** | 0 | 更多详情可见[可视性和遮挡剔除](../visibility-and-occlusion-culling/index.md)的层级Z缓冲遮挡部分。 |
...
- **比例（Scale）：** VR场景中的物体比例应尽可能地模拟真实情况。物体过大或过小可能会导致用户产生困惑，或者产生[晕眩](../../../sharing-and-releasing-projects/developing-for-xr-experiences/getting-started-with-xr-development/xr-best-practices/index.md#vr%E4%B8%8E%E6%A8%A1%E6%8B%9F%E6%99%95%E7%9C%A9%E7%97%87)。
...
- **VR和透明度：** 在3D图形中，渲染透明度的开销极其昂贵，因为一般来说，透明效果每帧都需要重新计算一下，以检查是否有所变化。由于要重新评估，在VR中渲染透明度的开销变得过于昂贵，得不偿失。不过为了避免这个问题，你可以使用 **DitherTemporalAA** 材质函数。该材质函数可以让材质看似使用了透明效果，还能避免自排序等常见的透明度问题。 (convert:false) *点击查看大图。*
...
- **屏幕空间全局光照：** 屏幕空间技术可以在HMD中的两只眼睛之间产生显示差异。这些差异可能会导致用户不适。请参阅[光照类型](../../../sharing-and-releasing-projects/developing-for-xr-experiences/getting-started-with-xr-development/xr-best-practices/index.md)，了解VR中建议改用的光照类型。
