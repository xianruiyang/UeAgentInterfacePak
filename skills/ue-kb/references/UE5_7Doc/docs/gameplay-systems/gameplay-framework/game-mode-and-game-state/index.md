...
source_url: "https://dev.epicgames.com/documentation/unreal-engine/game-mode-and-game-state-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "Gameplay系统", "Gameplay框架", "Game Mode 和 Game State"]
---

# Game Mode 和 Game State

> 路径：虚幻引擎5.7文档 / Gameplay系统 / Gameplay框架 / Game Mode 和 Game State

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/game-mode-and-game-state-in-unreal-engine
...
- EnteringMap
  ...
  是初始状态。Actor 尚未进行 tick，世界场景尚未完整初始化。内容完整加载后将过渡到下个状态。
- WaitingToStart
...
- WaitingPostMatch
  ...
  。Actor 仍在 tick，但新玩家无法加入。地图转换开始时它将过渡到下个状态。
- LeavingMap

  是正常流程中的最后一个状态，进入时将调用

  HandleLeavingMap

  。转换到新地图时比赛将保持在此状态中，进入新地图时将过渡回到
...
- URL 可被传到可执行文件，强制游戏加载时带特定选项。使用 `game` 选项设置游戏模式。查看 [命令行参数](../../../cpp-programming/programming-in-the-unreal-engine-architecture/command-line-arguments/index.md)中的详细内容。
...
> [!NOTE]
> 请查阅 [设置 Game Mode](../../../gameplay-tutorials/setting-up-a-game-mode/index.md) 文档中设置 Game Mode 的范例。
