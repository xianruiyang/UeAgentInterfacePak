---
title: "使用重定向动画"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/using-retargeted-animations-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "为角色和对象制作动画", "骨架网格体动画系统", "动画操作指南和示例", "使用重定向动画"]
---

# 使用重定向动画

> 路径：虚幻引擎5.7文档 / 为角色和对象制作动画 / 骨架网格体动画系统 / 动画操作指南和示例 / 使用重定向动画

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/using-retargeted-animations-in-unreal-engine
...
3. 在 **内容浏览器** 中的 **Content/InfinityBladeWarriors/Character/CompleteCharacters** 文件夹下，打开 **SK_Mannequin_Skeleton** 资源。
...
9. 在 **选择骨架（Select Skeleton）** 窗口中，你应该可在窗口上部看到目标骨架，选中它并单击 **重定位（Retarget）**。
   ...
   > [!NOTE]
   > 为确保生成最佳重定位后的动画，**目标** 骨架应具有与源骨架相似的基本姿势。 如上，两个骨架都呈A字姿势，但是，你也可能碰到一个角色呈A字姿势，而另一个角色呈T字姿势的情况。 你可以使用[重定位基本姿势](../../animation-assets-and-features/animation-sequences/retarget-manager/index.md)在重定位动画前修改姿势。
