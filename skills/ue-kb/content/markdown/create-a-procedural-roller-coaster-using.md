# Create a Procedural Roller Coaster Using

# Create a Procedural Roller Coaster Using

## 知识目标

- 围绕“Create a Procedural Roller Coaster Using”整理 PCG 视频中的输入数据、图表规则、关键节点、参数风险和最终生成结果。
...
- `PCG`、`Blueprint`、`Static Mesh`、`Mesh`、`Point Filter`、`Spline`、`Transform`、`Point`、`Attribute`、`Actor`、`Component`、`Spawn`、`Bounds`、`Density`、`Random`、`Graph`、`Landscape`、`points`、`PCG Graph`、`PCG Component`、`Spline Sampler`、`Volume`、`Static Mesh Spawner`、`Spline Mesh`

## 分段知识

### 00:00:00-00:04:16 Spline 采样与路径生成
...
### 00:04:16-00:09:10 Spline 采样与路径生成
...
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 复现要点：Spline 流程要单独核对采样间距、切线、端点、交叉点和生成网格的对齐方式。
- 核对对象：`Point`、`Attribute`、`Spline`、`Spline Sampler`、`Volume`、`Static Mesh`、`Static Mesh Spawner`、`Spline Mesh`、`Transform Points`、`Filter`。
...
### 00:09:10-00:13:54 Spline 采样与路径生成
...
- 核对对象：`Point`、`Attribute`、`Spline`、`Spline Sampler`、`Volume`、`Actor`、`Create Spline`、`Partition`、`采样`、`生成`。
...
### 00:13:54-00:18:46 Spline 采样与路径生成
...
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 复现要点：Spline 流程要单独核对采样间距、切线、端点、交叉点和生成网格的对齐方式。
- 核对对象：`Point`、`Attribute`、`Spline`、`Spline Sampler`、`Transform Points`、`Filter`、`Partition`、`Random`、`采样`、`生成`。
...
### 00:18:46-00:22:21 Spline 采样与路径生成
...
- 复现要点：Static Mesh Spawner 用于高效实例化；Spawn Actor/Blueprint 用于需要逻辑的对象，成本和生命周期不同。
- 复现要点：Spline 流程要单独核对采样间距、切线、端点、交叉点和生成网格的对齐方式。
- 核对对象：`Point`、`Spline`、`Spline Sampler`、`Blueprint`、`Create Spline`、`Data Asset`、`采样`、`生成`。
...
### 00:22:21-00:22:39 Spline 采样与路径生成
