---
title: "状态机"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/state-machines-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "为角色和对象制作动画", "骨架网格体动画系统", "动画蓝图", "状态机"]
---

# 状态机

> 路径：虚幻引擎5.7文档 / 为角色和对象制作动画 / 骨架网格体动画系统 / 动画蓝图 / 状态机

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/state-machines-in-unreal-engine
...
- 状态机需要在
  ...
  中创建，因此你应该懂得如何使用动画蓝图及其

  操作界面

  。
- 你的项目包含带有运动组件的角色，以便构建状态来对输入做出反应。若没有这种角色，你可以使用
...
状态机在[动画图表](../graphing-in-animation-blueprints/index.md#%E5%8A%A8%E7%94%BB%E5%9B%BE%E8%A1%A8)中创建。要创建一个状态机，请右键点击 **动画图表（Anim Graph）**，然后选择 **状态机（State Machines）> 添加新状态机（Add New State Machine）** 。将其连接到 **Output Pose**。

![添加新状态机](../../../../../assets/images/f7/f7415e026e6563474d5f0a8203624891c9ba34a3dd01a62993c56cf569d22a56.jpg)

状态机是动画图表中的子图表，因此你可以在 **我的蓝图（My Blueprint）** 面板中看到状态机图表。双击它打开状态机。

![新状态机](../../../../../assets/images/da/da0e22861be5adece15d74f2d1a722e78f9c54d006e89a5d0c0c220cf1b74053.png)
...
![状态进入点](../../../../../assets/images/eb/ebdff2f9a56e17af0209173a343f2d79a8a75d67cf4f11e7e499da19acc335ec.png)
...
- 右键点击状态机图表，然后选择 **添加状态（Add State）** 。
  ...
  ![添加状态](../../../../../assets/images/ad/ad83b8c343f0a5327854cd6ffc9421b5574aa898744bb140fb668769e072b3ba.png)
- 点击状态（或进入输出）的边框并拖出，然后松开鼠标，并选择 **添加状态（Add State）** 。这还会将其连接到带有[转换](#%E8%BD%AC%E6%8D%A2)的之前状态。

  ![拖动添加状态](../../../../../assets/images/b3/b31cb303a1b521ccafb92b9cd2ce749af57b1ec6c85abd6f1e7791ad3ff281d6.png)
- 将 **动画资产（Animation Asset）** 从 **内容浏览器（Content Browser）** 或 **资产浏览器（Asset Browser）** 拖入状态机图表中。这还会将动画添加到状态，并将其连接到其 **Output Pose**。

  ![拖放添加状态](../../../../../assets/images/99/997d7d1bb7c4877e6451d7986e3bf459d4da706b5af2d9feff340589cd904782.jpg)

状态机可以根据需要包含任意数量的状态。它们会显示为状态机下的子图表。

![我的蓝图面板中的状态](../../../../../assets/images/b6/b6342922e75889a0bd1c3ba2dd2770aeccb0e6d6b578e972da966c0ae257674c.jpg)

### 编辑状态

要查看状态的内部操作，可以在 **我的蓝图（My Blueprint）** 面板中双击它，或在 **状态机（State Machine）** 图表中双击该节点本身。这将打开该状态。

![打开状态](../../../../../assets/images/05/05aa6c37ba6f8abb22f28e338715daad47316c7fd2fa2904979566693a7c6c2a.png)
...
| --- | --- |
| **名称（Name）** | 所选状态的名称。 |
| **进入状态事件（自定义蓝图事件）（Entered State Event (Custom Blueprint Event)）** | 通过 **自定义蓝图事件（Custom Blueprint Event）** 字段中使用的名称创建[骨架通知](../../animation-assets-and-features/animation-sequences/animation-notifies/index.md#%E9%AA%A8%E6%9E%B6%E9%80%9A%E7%9F%A5)。此通知将在状态变为活动并开始转换时执行。与普通骨架通知一样，你可以在动画蓝图的 **事件图表（Event Graph）** 中创建事件来访问该事件。 进入状态自定义蓝图事件 |
...
如需详细了解 **转换** 和 **转换规则**，请参阅[转换](../transition-rules/index.md)页面。


- [转换规则](../transition-rules/index.md)
...
有多种方法可以使用导管。其中一个例子是，使用导管来分散状态机的进入点。然后，你可以使用导管中的转换，选择哪个状态应该作为默认状态开始。重新初始化状态机时，如果你要使用另一个动画来覆盖它，例如[动画蒙太奇](../../animation-assets-and-features/animation-montage/index.md)，此示例可能很有用。

> 图片已省略：导管连接

> [!NOTE]
> 以上示例需要在[状态机细节面板](#%E7%8A%B6%E6%80%81%E6%9C%BA%E5%B1%9E%E6%80%A7)中启用 **允许导管进入状态（Allow Conduit Entry States）** 。

导管包含自身的[转换规则](../transition-rules/index.md#%E8%BD%AC%E6%8D%A2%E8%A7%84%E5%88%99)，若要找到这些规则，可以双击导管节点，或从"我的蓝图（My Blueprint）"面板打开导管图表。默认情况下，导管转换规则将返回false。大部分情况下，你可能只想启用 **可以进入转换（Can Enter Transition）** ，并在进出导管的各个转换上创建转换规则逻辑。

> 图片已省略：允许导管直通

请参阅[转换规则](../transition-rules/index.md)页面，了解有关转换和转换规则的更多信息。
...
| **创建通知元数据（Create Notify Meta Data）** | 在转换规则中使用[动画通知函数](../../animation-assets-and-features/animation-sequences/animation-notifies/index.md#%E5%8A%A8%E7%94%BB%E9%80%9A%E7%9F%A5%E5%87%BD%E6%95%B0)时，启用该属性就可以将所有相关数据发送到这些通知函数。如果禁用该属性，则所有通知函数都无法运行。 |
| **允许导管进入状态（Allow Conduit Entry States）** | 启用该属性将允许[导管](#%E5%AF%BC%E7%AE%A1)用作进入状态，从而允许不同的默认状态，具体取决于导管的转换规则。 |
