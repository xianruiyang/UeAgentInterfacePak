---
title: "多播委托"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/multicast-delegates-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "用C++编程", "委托", "多播委托"]
---

# 多播委托

> 路径：虚幻引擎5.7文档 / 用C++编程 / 委托 / 多播委托

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/multicast-delegates-in-unreal-engine

多播委托拥有大部分与单播委托相同的功能。它们只拥有对对象的弱引用，可以与结构体一起使用，可以四处轻松复制等等。就像常规委托一样，多播委托可以远程加载/保存和触发；但多播委托函数不能使用返回值。它们最适合用来四处轻松传递一组委托。

## 声明多播委托

多播委托在声明方式上与[声明标准委托](../index.md)相同，只是前者使用特定于多播委托的宏变体。
...
| `AddLambda()` | 添加一个函子。这通常用于那些不会捕获指针的lambda函数。 |
| `AddSPLambda()` | 添加一个函子。它只有在对目标共享指针的弱引用有效的情况下才会被调用。 |
| `AddWeakLambda()` | 添加一个函子。它只有在对UObject的弱引用有效的情况下才会被调用。 |
