---
title: "编辑动画层"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/editing-animation-layers"
breadcrumbs: ["虚幻引擎5.7文档", "为角色和对象制作动画", "骨架网格体动画系统", "动画操作指南和示例", "编辑动画层"]
---

# 编辑动画层

> 路径：虚幻引擎5.7文档 / 为角色和对象制作动画 / 骨架网格体动画系统 / 动画操作指南和示例 / 编辑动画层

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/editing-animation-layers

动画层编辑功能允许你在虚幻引擎中修改任何现有的[动画序列](../../animation-assets-and-features/animation-sequences/index.md)，作为 **附加层轨迹（Additive Layer Tracks）** 的一部分。 借助动画层编辑功能，你可以使用现有动画并进行调整，制作该动画的不同副镜头，也可以基于原动画创建一段独立的、有自身动作的新动画。
...
2. 在 **FirstPerson_Reload** 中，在第0帧处暂停动画，在 **骨架树（Skeleton Tree）** 中单击 **upperarm_l** 骨骼，然后单击 **关键帧（Key）** 按钮。

   此操作会将upperarm_l的当前位置的 **变换（Transform）**、**旋转（Rotation）** 和 **缩放（Scale）** 关键帧添加到 **附加层轨迹（Additive Layer Tracks）** 部分。
3. 将时间轴向前移动到第 **15** 帧。
4. 将 **upperarm_l** 骨骼旋转 **-70** 度，然后按 **关键帧（Key）** 按钮。

   此操作会为动画第15帧处的骨骼旋转添加一个关键帧。
5. 将时间轴向前移动到第 **20** 帧，将 **upperarm_l** 骨骼旋转 **70** 度至其默认位置，然后按 **关键帧（Key）** 按钮。

   在这20帧动画期间，手臂将先向下移动，然后向上移动到其默认位置。
6. **右键单击** 时间轴标记并 **删除第20帧到第92帧**。

   此操作将删除该动画剩余的帧，仅保留填弹动作。
7. 在 **内容浏览器** 中，**右键单击** **FirstPerson_Reload** 动画并选择 **创建动画蒙太奇（Create AnimMontage）**。

   当玩家在游戏进程中按下指定的键时，我们将使用此动画蒙太奇来播放新的填弹动画。
8. 打开新的动画蒙太奇，然后更改插槽（改为使用 **DefaultGroup.Arms** 插槽）。

   如下所示，指定给模板角色的动画蓝图已设置为使用[插槽节点](../../animation-blueprints/animation-blueprint-nodes/animation-blueprint-blend-nodes/index.md#slot)来播放指定给"Arms"插槽的任何动画蒙太奇。
