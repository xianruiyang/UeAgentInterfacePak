---
title: "控件交互组件"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/umg-widget-interaction-components-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "创建用户界面", "构建用户界面", "构建你的UI", "控件交互组件"]
---

# 控件交互组件

> 路径：虚幻引擎5.7文档 / 创建用户界面 / 构建用户界面 / 构建你的UI / 控件交互组件

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/umg-widget-interaction-components-in-unreal-engine

如您使用 [控件组件](../index.md) 显示以 3D 形式存在于游戏世界中的 UI，还需要玩家与此控件进行交互，可通过 **控件交互（Widget Interaction）** 组件来实现交互。

控件交互组件执行 [光线投射](../../../../gameplay-systems/physics/traces-with-raycasts/traces-tutorials/index.md)，确定它是否命中世界场景中的控件组件。如命中，可设置规则确定与其交互的方式。 交互通过模拟定义的按键来执行。例如一个按钮可通过鼠标左键点击，即可告知其他形式的输入模拟一次鼠标左键点击（控制器按钮按下、运动控制器扳机键按下等）。
...
通常需要将控件交互组件添加到玩家 [Pawn](../../../../gameplay-systems/gameplay-framework/pawn/index.md) 或来自 [组件窗口](../../../../blueprints-visual-scripting/specialized-blueprint-visual-scripting-node-groups/components-window/index.md) 的 [角色](../../../../gameplay-systems/gameplay-framework/pawn/characters/index.md) 类。
