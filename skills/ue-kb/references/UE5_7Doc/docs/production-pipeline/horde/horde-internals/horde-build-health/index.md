---
title: "Horde构建健康状况"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/horde-build-health-for-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "建立你的开发流程", "Horde", "Horde内部机制", "Horde构建健康状况"]
---

# Horde构建健康状况

> 路径：虚幻引擎5.7文档 / 建立你的开发流程 / Horde / Horde内部机制 / Horde构建健康状况

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/horde-build-health-for-unreal-engine
...
- Type
  ...
  ：一个用于标识问题类型的字符串。指纹只会与同类型的其他指纹匹配。最常遇到的问题类型之一是
...
- SummaryTemplate
  ...
  ：一个字符串，用于在Horde操作面板和通知中填充问题的描述。可能包含占位符字符串，当指纹合并在一起时，这些占位符字符串将被展开（请参阅
...
- Keys
  ...
  ：用于对指纹进行匹配和分组的标识数据片段，例如与错误相关的文件名。每个键都有名称和类型。
- RejectKey
...
- Metadata
  ...
  ：任意键/值数据，可以聚合用于摘要模板和调试。
- ChangeFilter

  ：一组用分号隔开的通配符，用于指定可被认为是导致此错误的文件，例如对于C++源文件，可以是
...
- {Severity}
  ...
  ：根据分组到该问题中的诊断的最高严重程度，扩展为字符串 'Warnings'或'Errors'。
- {Files}
...
工作流程通过流配置文件中的[`Workflows`](../../horde-configuration/horde-schema/index.md#streams)属性进行配置，作业通过作业模板的[`WorkflowId`](../../horde-configuration/horde-schema/index.md#templaterefconfig)属性配置为使用作业。

作业中的各个节点可以通过在BuildGraph脚本中使用[注解](#%E6%B3%A8%E8%A7%A3)，配置为使用工作流程。
...
- AutoAssign
  ...
  ：是否自动分配只能由一个用户导致的问题，或者与修改的文件有明确关联的问题。
- AutoAssignToUser

  ：将此步骤中的问题自动分配给由提供参数所指定的Perforce用户。
- NotifySubmitters

  ：是否通知在构建从成功转变为失败期间的所有提交者，允许他们主动站出来对问题承担责任。
- IssueGroup

  ：指定一个后缀，将该后缀附加到从该节点创建的问题的
...
- BuildBlocker
  ...
  ：该节点中的失败是否应被视为构建阻碍。被识别为构建阻碍的问题在Slack通知中有一个特殊标记。
