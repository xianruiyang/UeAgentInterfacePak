...
---
title: "NDisplay工作流程"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/ndisplay-workflows-for-smpte-2110-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "使用媒体", "媒体集成", "使用nDisplay在多显示屏上进行渲染", "将SMPTE 2110用于nDisplay", "NDisplay工作流程"]
---

# NDisplay工作流程

> 路径：虚幻引擎5.7文档 / 使用媒体 / 媒体集成 / 使用nDisplay在多显示屏上进行渲染 / 将SMPTE 2110用于nDisplay / NDisplay工作流程

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/ndisplay-workflows-for-smpte-2110-in-unreal-engine
...
![SMPTE ST 2110 ICVFX摄像机流送。](../../../../../../assets/images/27/273257cd0005c1db6e0d6fdc7514fac4240e6ef370cb587021e3d02f2f1472e8.jpg)
...
1. 转到**内容浏览器**，搜索后打开你的nDisplay配置。 示例中的文件名为**NDC_MyStage**：

   ![image alt text](../../../../../../assets/images/c9/c948fd8467d2d6e472b81f4da155cf7263620a054f956e53a960a61abe19b8fd.png)
...
2. 同时还存在两台ICVFX摄像机。 针对每台摄像机，请点击**+添加（+Add）**，然后选择**添加新群集节点（Add New Cluster Node）**，以创建用于渲染该摄像机的节点。
   ...
   ![添加按钮的选项](../../../../../../assets/images/cb/cb76fe2a44dcd2b311556dadfbbdeb631e30d5a14e8ee33287b05da0e42bc333.png)
...
3. 配置完成后，你的**添加新群集节点（Add New Cluster Node）**对话框应与下图类似。
   ...
   ![添加新群集节点对话框的选项](../../../../../../assets/images/3b/3bc0ed09c5f5536746011219e916451e26f95d83363e9e499adb5ac7ddeb0d8a.png)
4. 创建节点后，你的群集应与下图类似：

   ![群集节点](../../../../../../assets/images/6e/6e0005d8a35986d3773a1b6d3843c334eb32788d52162587e8916cdae7a2f936.jpg)
...
1. 转到舞台大纲视图，选择**ICVFXCameraA**。
   ...
   ![选择ICVFXCameraA](../../../../../../assets/images/58/588ead3f4e5bc80bc864f9b0e13a898c9ee915d1db5c32b1579b3112517a4119.png)
2. 在**细节（Details）**面板中，找到**媒体（Media）**分段：

   ![细节面板中的媒体分段](../../../../../../assets/images/47/4788cfcef56139964f5940e87eaa3af56b0da9e2859c6d9cb78dc3816f0f8db3.png)
3. 勾选**启用（Enable）**复选框。
4. 添加一个**媒体输出群组（Media Output Group）**。 你将在此处配置渲染此内视锥体的节点，以及它的共享方式。
5. 你需要用来渲染此摄像机的节点是**ICVFXCameraA** 的**Node_CamA**。 如果你要配置ICVFXCameraB，请改为选择Node_CamB。
...
![SMPTE 2110 nDisplay ICVFX输出设置项的配置。](../../../../../../assets/images/81/8127f4a463caa4bda8c678e4a9698480f2a85cd41fceb34c261ac64f96e1dae8.jpg)
...
2. 添加接收此输出的**群集节点（Cluster Nodes）**。 在本案例中，你需要的是所有驱动着墙体的节点（Node_1、Node_2和Node_3）。
...
> 图片已省略：SMPTE 2110 nDisplay ICVFX输入设置项的配置。

SMPTE 2110 nDisplay ICVFX输入设置项的配置。

你的ICVFXCameraA现已配置为从一个渲染节点共享到群集。 媒体设置应类似于下图：

> 图片已省略：SMPTE 2110 nDisplay ICVFX媒体设置项的配置。
...
1. 媒体输出群组
   ...
   - 渲染ICVFXCameraB的**群集节点（Cluster Node）**是Node_CamB。
...
> 图片已省略：SMPTE ST 2110 ICVFX同步摄像机流送。
...
示例中有三个节点需要发送，分别是Node_1、Node_2和Node_3。 由于不存在分辨率限制，你可以尽可能缩小节点的窗口，从而仅发送最低限度的所需像素。 这时，你应添加一条限制，从而将Node_1和Node_2的窗口设置为相同大小。 当作它们使用了相同的LED处理器。 Node_3的流送将不具有此限制，窗口大小也会不同。

#### 节点配置 – Node_1和Node_2

从Node_1和Node_2的配置开始。 在原始配置中：

- Node_1使用全屏节点，窗口大小为7680x2160，视口大小为2640x1408。
- Node_2使用全屏节点，窗口大小为3840x2160，视口大小为3344x1408。

> 图片已省略：Node_1的原始示例配置设置项。

Node_1的原始示例配置设置项。

> 图片已省略：VP_W1视口中使用原始示例设置的Node_1流送。

VP_W1视口中使用原始示例设置的Node_1流送。

> 图片已省略：VP_W2视口中使用原始示例设置的Node_2流送。

VP_W2视口中使用原始示例设置的Node_2流送。

1. 由于视口VP_W2最大，因此请使Node_1和Node_2使用相同的设置，并使用VP_W2的3344x1408大小。
2. 启用**无头渲染（Headless Rendering）**设置，以将两个节点设置为屏幕外，并禁用**全屏（Fullscreen）**设置。

> 图片已省略：Node_1和Node_2的修改设置。

Node_1和Node_2的修改设置。

> 图片已省略：修改设置后，VP_W1视口内的Node_1流送。

修改设置后，VP_W1视口内的Node_1流送。

> 图片已省略：修改设置后，VP_W2视口内的Node_2流送。

修改设置后，VP_W2视口内的Node_2流送。

这一改动为Node_1的流送增加了额外的像素，从而使其与Node_2的流送大小相匹配。 当视口不完全相等时，必须考虑其在各个节点上的组织形式，以尽量减少带宽浪费。

现在请配置各节点的媒体输出。 你可以在节点级别（最终后备缓冲区）和视口级别配置媒体。 必须要在节点上进行此配置，以发送最终合成和扭曲后的结果。

1. 选择Node_1并找到其媒体（Media）分段：
...
4. 将媒体输出配置为**NVIDIA Rivermax Output**，并配置相应的设置：
   ...
   - **流送地址（Stream Address）**：*唯一的Node_1流送地址*。 请为本例使用**225.1.2.1**，并递增所有节点的八位数最后一位，从而让每个节点都拥有唯一的组播地址。 请确保此处使用的组播群组尚未在网络中使用，以避免争用。
...
Node_1 的配置就此完成，其设置应如下所示：

> 图片已省略：针对nDisplay ICVFX摄像机流送和同步输出示例的Node_1媒体设置。

针对nDisplay ICVFX摄像机流送和同步输出示例的Node_1媒体设置。

Node_1配置完成后，请继续配置Node_2。 其配置应与Node_1相同，但**流送地址（Stream Address）**除外，因为必须**唯一**。

1. 将Node_2设置为与Node_1相同。
2. 将Node_2的**流送地址（Stream Address）**设为225.1.2.2。

Node_2的最终配置如下：

> 图片已省略：针对nDisplay ICVFX摄像机流送和同步输出示例的Node_2媒体设置。

针对nDisplay ICVFX摄像机流送和同步输出示例的Node_2媒体设置。

#### 节点配置 – Node_3

现在请配置Node_3。 在上文的原始示例中，两个视口之间存在间隙。

> 图片已省略：原始的示例设置中视口间存在间隙、
...
然后，像对Node_1和Node_2所做的那样，将此节点配置为无头节点，并将其窗口大小调整为包含两个视口所需的最小大小。
...
> 图片已省略：VP_C1和VP_C2视口中显示的Node_3流送。

VP_C1和VP_C2视口中显示的Node_3流送。

接下来，配置Node_3的媒体输出配置。 同样地，采用与Node_1和Node_2相同的设置，但流送地址除外，它需要在群集中唯一。

1. 将Node_3设置为与Node_1和Node_2相同。
2. 将Node_3的**流送地址（Stream Address）**设为225.1.2.3。

配置完成后，你的群集现在将使用5个组播群组：

1. 225.1.1.10:50000：ICVFXCameraA
2. 225.1.1.11:50000：ICVFXCameraB
3. 225.1.2.1:50000 ：Node_1
4. 225.1.2.2:50000 ：Node_2
5. 225.1.2.3:50000 ：Node_3

Node_3的最终配置如下：

> 图片已省略：针对nDisplay ICVFX摄像机流送和同步输出示例的Node_3媒体设置。

针对nDisplay ICVFX摄像机流送和同步输出示例的Node_3媒体设置。
