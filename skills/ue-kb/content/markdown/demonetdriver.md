...
---
title: "DemoNetDriver和流送器"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/demonetdriver-and-streamers-in-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "Gameplay系统", "联网和多人游戏", "重播系统", "DemoNetDriver和流送器"]
---

# DemoNetDriver和流送器

> 路径：虚幻引擎5.7文档 / Gameplay系统 / 联网和多人游戏 / 重播系统 / DemoNetDriver和流送器

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/demonetdriver-and-streamers-in-unreal-engine
...
| 关卡组 | 添加至该组的关卡 | 行为 |
| --- | --- | --- |
| **静态关卡（Static Levels）** | 非持续关卡并且带有 `IsStatic` 布尔值标记的关卡。 | 不受游戏影响，并且在实时游戏和回放中都会显示。 |
| **动态源关卡（Dynamic Source Levels）** | 持续关卡以及任何 `IsStatic` 布尔值变量设为false的子关卡。 | 受游戏影响。在回放时会被隐藏，但是游戏仍然正常运行。 |
| **动态复制关卡（Dynamic Duplicate Levels）** | 加载时由动态源关卡生成的复制。不存在于专门的服务器或者编辑器模式中。 | 在游戏时隐藏。回访会在这些关卡中播放，然后关卡便清空。 |
...
HTTP流送器会通过[HTTP流送REST API](http-streamer-rest-api/index.md)与自定义编写的回放服务器沟通，使用GET和POST方法来将数据作为二进制文件或者JSON格式的字符串发送。要设立你自己的回放服务器，需要先建立你的URL。你的项目DefaultEngine.Ini文件包含了回放服务器的URL，位于 `[HttpNetworkReplayStreaming]` 分段，变量为 `ServerURL` 。 `ServerURL` 的格式应该为 "http://replay.yourgame.com/"。末尾的 "/" 非常重要，因为HTTP流送器不会认为它应该更改URL的格式。
