---
title: "Horde存储"
source_url: "https://dev.epicgames.com/documentation/unreal-engine/horde-storage-for-unreal-engine"
breadcrumbs: ["虚幻引擎5.7文档", "建立你的开发流程", "Horde", "Horde内部机制", "Horde存储"]
---

# Horde存储

> 路径：虚幻引擎5.7文档 / 建立你的开发流程 / Horde / Horde内部机制 / Horde存储

> 原始页面：https://dev.epicgames.com/documentation/unreal-engine/horde-storage-for-unreal-engine
...
- 类型（Type）
  ...
  ：由一个GUID和整型版本号表示，用于区分可能具有特定序列化格式但数据相同的负载。
- 负载（Payload）

  ：一个字节数组。Blob意味着完全读入内存，因此负载通常限制在几百KB以内。对于较大的负载，可以使用实用工具库，通过静态分块或内容定义的分块将其拆分为更小的blob。
- 引用（References）
...
- 引用
  ...
  是指向blob存储的强引用，并且充当垃圾回收器的根节点。引用可以设置为在固定时间到期，或者在特定时间段内未被检索后到期。这对实现缓存非常有用。
- 别名

  是对blob的弱引用。可能存在多个名称相同的别名，用户可以通过特定的别名来查询一个或多个blob。别名关联用户指定的优先级，使用者可以按照优先级顺序来查询别名。
...
- 逻辑存储模型通过
  ...
  接口声明，这是与存储系统进行交互的主要方式。在这一层，blob通过
...
- 物理存储模型通过
  ...
  接口声明，该接口负责通过网络将数据发送到存储服务实现。

  - HttpStorageBackend

    通过HTTP将数据上传到Horde服务器。
  - FileStorageBackend
